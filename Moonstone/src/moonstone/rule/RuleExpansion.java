...
public class RuleExpansion {
	...
	public RuleExpansion(Rule rule, Annotation matched, int index) {
		...
		// 7/23/2018
		if (MoonstoneRuleInterface.RuleEditor.isPrintRuleStatistics()) {
			HUtils.incrementCount(rule.grammar.ruleExpansionCountHash, rule);
		}
	}
	...
	public RuleExpansion(RuleExpansion expansion, Annotation matched, int index) {
		if (expansion != null && expansion.rule.isDoDebug()) {
			...
			if (this.rule != null) {
				for (int i = 0; i < this.rule.getPatternListCount(); i++) {
					Annotation a = this.matchedAnnotations[i];
					StringConstant c = (StringConstant) a.getConcept();
					if (a != null && ":POOR-COMPLIANCE-WITH-TREATMENT:".equals(c.getName())) {
						break;
					}
				}
			}
		...
		this.matchedAnnotations[index] = matched;
		if (expansion.rule.isDoDebug() && "has".equals(matched.getText())) {
			int x = 1;
		}
		this.isFullyMatched = checkIsFullyMatched();
		this.isValid = this.checkIsValid();
	...
	public Annotation createAnnotation() {
		...
		if (this.isFullyMatched()) {
			Vector<Annotation> sources = VUtils.arrayToVector(this.matchedAnnotations);
			...
			if (this.getGrammar() instanceof NarrativeGrammar) {
				...
				annotation = new NarrativeAnnotation(this.sentenceAnnotation, this.rule, sources, true);
			} else if (this.getGrammar() instanceof DocumentGrammar) {
				annotation = new moonstone.annotation.DocumentAnnotation(this.sentenceAnnotation, this.rule, sources,
						true);
			} else if (this.getGrammar() instanceof StructureGrammar) {
				annotation = new StructureAnnotation(this.sentenceAnnotation, this.rule, sources, true);
			}
	...
	// 2/13/2018
	public boolean allMatchedAnnotationsAreValid() {
		for (int i = 0; i < matchedAnnotations.length; i++) {
			if (!matchedAnnotations[i].isValid()) {
				return false;
			}
		}
		return true;
			...
			if (current != null) {
				...
				if (!(this.getGrammar() instanceof DocumentGrammar)) {
					...
					if (current != null && last != null) {
						...
						if (!SeqUtils.disjoint(last.getRelativeTokenStart(), last.getRelativeTokenEnd(),
								current.getRelativeTokenStart(), current.getRelativeTokenEnd())) {
							this.invalidReason = HasOverlap;
							return false;
						}
						if (rule.isOrdered() || rule.isJuxtaposed()) {
							if (!(last.getRelativeTokenEnd() < current.getRelativeTokenStart())) {
								this.invalidReason = NotOrdered;
								return false;
							}
							if (itemsJuxtaposed) {
								if (rule.isDispositionCreateAnnotation() && !rule.isPermitInterstitialAnnotations()
										&& this.grammar.interpretedAnnotationsExist(last.getRelativeTokenEnd() + 1,
												current.getRelativeTokenStart() - 1)) {
									this.invalidReason = HasInterstitial;
									return false;
								}
								if (rule.isJuxtaposed()) {
									if (last.getWordTokenEnd() != current.getWordTokenStart() - 1) {
	...
	public String toString() {
		...
		str += "(Start=" + this.wordTokenStart + ",End=" + this.wordTokenEnd + ",%=" + this.wordTokenCoverage + ")";
		str += ">";
		return str;
	}
	...
	public void calculateWordTokenValues() {
		...
		this.wordTokenCoverage = (float) this.totalWordTokenCount / (float) this.coveredWordTokenCount;
	}
	...
	public boolean isValidPerRuleConstraints() {
		if (this.variables == null) {
			this.variables = Annotation.gatherPositionalPhraseVariables(VUtils.arrayToVector(this.matchedAnnotations));
	...
	// 4/25/2018
	public boolean fullyMatchedEquals(Object o) {
		if (o instanceof RuleExpansion) {
			int x = 2;
			RuleExpansion other = (RuleExpansion) o;
			if (this.getRule() == other.getRule() && this.isFullyMatched && other.isFullyMatched) {
				for (int i = 0; i < this.matchedAnnotations.length; i++) {
					if (this.getMatchedAnnotation(i) != other.getMatchedAnnotation(i)) {
						return false;
					}
				}
				return true;
