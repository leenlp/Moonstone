...
public class Rule extends Information {
	...
	public Rule(Grammar grammar, Sexp sexp, Vector pattern, Vector patternLists) {
		this(grammar, sexp, pattern, patternLists, null);
	}

	public Rule(Grammar grammar, Sexp sexp, Vector pattern, Vector patternLists, Sexp ruleFileProperties) {
		...
		// 5/15/2018 to uniquify rules that have the same name in Onyx pneumonia set...
		// 6/15/2018 Removed -- messed up EBL learning
		// str += "_" + this.ruleIDNum;
		...
		if (Sexp.doAssocValue(sexp, "uri") != null) {
			this.setUri(Sexp.doAssocValue(sexp, "uri").toString());
		}
		...
		Sexp of = (Sexp) Sexp.doAssocValue(sexp, "output-format");
		if (of != null) {
			this.outputFormat = TLUtils.convertSexpToJVector(of);
		}

		// this.addToGrammarPatternConstantMap();

		if (grammar instanceof DocumentGrammar) {
			((DocumentGrammar) grammar).addGateRuleEquivalentConcepts(this);
		}

		// 9/19/2018: Map concepts to rules
		if (this.getGrammar() instanceof NarrativeGrammar) {
			this.getGrammar().addStringConstantRule(this);
		}
	...
	public Sexp toSexp(boolean useOrigSexp) {
		...
		try {
			...
			if (this.resultType != null) {
				lstr += "(stype \"" + this.resultType + "\") ";
			}

			if (this.phraseType != null) {
				lstr += "(ptype \"" + this.phraseType + "\") ";
			}
			...
			if (this.getFlags() != null) {
				lstr += "(flags " + StrUtils.stringListConcat(this.getFlags(), " ") + ") ";
			}
			if (this.patternLists != null) {
				String wstr = "(words ";
			...
			// 5/9/2017, for OWL grammar extraction
			if (this.propertyPredicates != null) {
				String ppstr = "(properties ";
				for (Vector ppred : this.propertyPredicates) {
					ppstr += "\n	(" + ppred.firstElement() + " \"" + ppred.lastElement() + "\")";
				}
				ppstr += ")\n";
				lstr += ppstr;
			}
			...
			// 1/12/2018
			if (this.testPredicates != null) {
				String tstr = "(tests ";
				for (Constraint c : this.testPredicates) {
					tstr += "\n	" + c.toLisp() + " ";
				}
				tstr += ")\n";
				lstr += tstr;
			...
			if (useOrigSexp) {
				...
				if (this.propertyPredicates == null && Sexp.doAssoc(origsexp, "properties") != null) {
	...
	public String toString() {
		return "<Rule=" + this.getRuleID() + ",Concept=" + this.getResultConcept() + ",Pattern="
				+ this.getPatternLists() + ",File=" + this.getSourceFilePath() + ">";
	}
	...
	public boolean contentEquals(Object o) {
		...
		Rule other = (Rule) o;
		if ((this.getResultConcept() != null && !this.getResultConcept().equals(other.getResultConcept()))
				|| (other.getResultConcept() != null && !other.getResultConcept().equals(this.getResultConcept()))) {
			return false;
		}
		if (this.getType() != other.getType()) {
			return false;
		}
		if (!this.getPatternLists().equals(other.getPatternLists())) {
			return false;
		}
		if ((this.getPropertyPredicates() != null
				&& !this.getPropertyPredicates().equals(other.getPropertyPredicates()))
				|| (other.getPropertyPredicates() != null
						&& !other.getPropertyPredicates().equals(this.getPropertyPredicates()))) {
			return false;
		}
		if (!Constraint.equals(this.getTestPredicates(), other.getTestPredicates())) {
			return false;
		}
		return true;
	}
	...
	public static Vector<String> getRuleIDs(Vector<Rule> rules) {
		...
		if (rules != null) {
			for (Rule rule : rules) {
				ids = VUtils.add(ids, rule.getRuleID());
			}
		}
		return ids;
	}
	...
	public void setSourceFilePath(String sourceFilePath) {
		...
		} else if (this.resultConcept != null) {
			String rc = this.resultConcept.toString().toLowerCase();
			if (rc.contains("throw") || rc.contains("capture")) {
				this.isCaptureRule = true;
			}
		}
	}

	public void setSourceFilePathXXX(String sourceFilePath) {
		this.sourceFilePath = sourceFilePath;
		if (sourceFilePath.toLowerCase().contains("capture")) {
			this.isCaptureRule = true;
		} else if (this.resultConcept != null && this.resultConcept.toString().toLowerCase().contains("throw")) {
			this.isCaptureRule = true;
		}
	}
	...
	public void addFlag(String flag) {
		this.flags = VUtils.addIfNot(this.flags, flag);
	}
	...
	public boolean matchesSourceRule(Rule source) {
		if (this == source) {
			return true;
		}
		if (source != null && source.getSourceRule() != null) {
			return this.matchesSourceRule(source.getSourceRule());
		}
		return false;
	}
	...
	public String toExcelString() {
		...
		sb.append(this.getSourceFilePath());
		return sb.toString();
	}

	public static Rule readFromExcelString(Grammar grammar, String estr) {
		Rule rule = null;
		try {
			StringBuffer sb = new StringBuffer();
			Vector<String> valuestrs = StrUtils.stringList(estr, Rule.ExcelPatternDelimiter);
			String sourcefile = valuestrs.elementAt(valuestrs.size() - 1);
			sb.append("'(");
			for (int i = 0; i < TSLInformation.getExcelAttributeMaxIndex(); i++) {
	...
	public Vector<String> getOutputFormat() {
		return outputFormat;
	}

	public void addToGrammarPatternConstantMap() {
		if (this.getResultConcept() instanceof StringConstant) {
			StringConstant c = (StringConstant) this.getResultConcept();
			for (Vector<Vector> plst : this.getPatternLists()) {
				for (Object o : plst) {
					if (o instanceof StringConstant && !(o == c)) {
						StringConstant psc = (StringConstant) o;
						psc.addSupportedGrammarConcept(c);
					}
				}
			}
		}
	...
	// 7/10/2018: TEST: I need a way to more uniquely identify rules.
	public Object getGrammarPatternHashKey() {
		return this.getPatternLists();
	}
