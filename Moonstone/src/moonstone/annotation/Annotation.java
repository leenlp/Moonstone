...
public class Annotation extends Information {
	...
	// the future. Handle them explicitly in
	...
	protected Vector<Annotation> formattedSourceAnnotations = null;
	...
	// 11/18/2017: Added comma to list so I can use in punctuation. This may cause
	...
	public Annotation(Annotation source) {
		super();
		this.initialize(this.getSentenceAnnotation(), this.getRule(), this.getChildAnnotations(), false);
	...
	public Annotation(WordSequenceAnnotation sentenceAnnotation, Rule rule, Vector<Annotation> sources,
			boolean doExpand) {
		super();
		int x = 1;
	...
	public void initialize(WordSequenceAnnotation sentenceAnnotation, Rule rule, Vector<Annotation> sources,
			boolean doExpand) {
		...
		if (sources != null) {
			...
			Collections.sort(this.textuallySortedSourceAnnotations, new Annotation.TokenIndexSorter());
			// 11/18/2017
			if (rule != null && rule.getOutputFormat() != null) {
				Vector<Annotation> formattedSources = null;
				for (int i = 0; i < sources.size(); i++) {
					Annotation child = sources.elementAt(i);
					String format = rule.getOutputFormat().elementAt(i);
					if ("*".equals(format)) {
						formattedSources = VUtils.add(formattedSources, child);
					} else if ("children".equals(format)) {
						formattedSources = VUtils.appendNew(formattedSources, child.getChildAnnotations());
					} else if ("formatted".equals(format)) {
						formattedSources = VUtils.appendNew(formattedSources, child.getFormattedSourceAnnotations());
					}
				}
				this.formattedSourceAnnotations = formattedSources;
			}
		}
		if (rule != null) {
			if (!processQuery()) {
			...
			if (rule.usesVariables()) {
				...
				if (rule.getLocalVariables() != null) {
					for (Variable lvar : rule.getLocalVariables()) {
						Object value = this.evalPatternRecursive(lvar.getValue(), this.getVariables());
		...
		if (this.textuallySortedSourceAnnotations != null) {
			this.numberOfChildren = this.textuallySortedSourceAnnotations.size();
			...
			this.string = this.sentenceAnnotation.getDocumentText().substring(this.textStart, this.textEnd + 1);
		...
		if (this.getRule() != null && this.getRule().isComplexConcept()) {
			x = 1;
		}
		Interpretation si = Interpretation.create(this, this.getCui(), this.getConcept(), this.getMacro(),
	...
	public static Vector<Variable> gatherPositionalPhraseVariables(Vector<Annotation> sources) {
		Vector<Variable> vars = new Vector();
		for (int i = 0; i < sources.size(); i++) {
			Variable var = new Variable("?" + i);
			Annotation source = sources.elementAt(i);
			Object value = (source.getValue() != null ? source.getValue() : source);
			var.bind(value);
	...
	public void doPostProcessing() {
		...
		if (interp != null) {
			if (this.headerContainsString("histor") || this.headerContainsString("past")) {
	...
	public void assignRelativeOffsets() {
		...
		//
		// 4/17/2015
		int ssi = this.getSentenceAnnotation().getSentence().getTokenStartIndex();
	...
	public Annotation(tsl.documentanalysis.document.Sentence sentence, TypeConstant gtype, TypeConstant stype,
			Vector<Token> tokens, Hashtable properties) {
		...
		this.string = sentence.getDocument().getText().substring(this.textStart, this.textEnd + 1);
	...
	public Annotation createTSLAnnotation(Grammar grammar, Annotation a1, Annotation a2, String atype, String relation,
			String property, String value) {
		if (a1 != null && a2 != null && a1.getSentenceAnnotation() == a2.getSentenceAnnotation()) {
			return new Annotation(grammar, a1, a2, atype, relation, property, value);
		}
		return null;
	}

	// 12/5/2014: For TSL-created annotations
	public Annotation(Grammar grammar, Annotation a1, Annotation a2, String atype, String relation, String property,
			String value) {
		...
		this.sentenceTokenStart = this.textuallySortedSourceAnnotations.firstElement().sentenceTokenStart;
		this.sentenceTokenEnd = this.textuallySortedSourceAnnotations.lastElement().sentenceTokenEnd;
		this.relativeTokenStart = this.textuallySortedSourceAnnotations.firstElement().getRelativeTokenStart();
		this.relativeTokenEnd = this.textuallySortedSourceAnnotations.lastElement().getRelativeTokenEnd();
	...
	public boolean processQuery() {
		if (this.getRule() != null && this.getRule().getQuerySentence() != null
			...
			Vector<RelationSentence> localSentences = Annotation
					.getRelationSentences(this.getTextuallySortedSourceAnnotations());
			if (localSentences != null) {
				Vector<Vector<Variable>> results = Query.doQuery(kb, qs, null, localSentences, true);
				if (results != null && results.size() > 0) {
					Vector<Variable> vars = Variable.createBoundVariables(qs.getVariables(), results.firstElement());
	...
	public boolean isLeaf() {
		return this.getLexicallySortedSourceAnnotations() == null && this.isUsedInHigherAnnotation();
	}
	...
	public boolean isTerminalWithInterpretation() {
		if (this.getProperty("cui") != null || this.getProperty("concept") != null || this.getProperty("macro") != null
				|| (this.getRule() != null && (this.getRule().getResultConcept() != null
						|| this.getRule().getResultCUI() != null || this.getRule().getResultMacro() != null))) {
			return true;
		}
		return false;
	}
	...
	public boolean hasMacro() {
		return this.semanticInterpretation != null && this.semanticInterpretation.hasMacro();
	}
	...
	public boolean hasTypeName(String tname) {
		return this.getSemanticInterpretation() != null && this.getSemanticInterpretation().hasTypeName(tname);
	}
	...
	public boolean isInterpreted() {
		...
		if (this.semanticInterpretation != null) {
			...
			if (si.getCui() != null || si.getConcept() != null || si.getType() != null || si.getMacro() != null) {
				return true;
			}
		}
		return false;
	...
	public void setLexicallySortedSourceAnnotations(Vector<Annotation> lexicallySortedSourceAnnotations) {
	...
	public Vector<Annotation> getFormattedSourceAnnotations() {
		return this.formattedSourceAnnotations;
	}
	...
	public void setTextuallySortedSourceAnnotations(Vector<Annotation> textuallySortedSourceAnnotations) {
	...
	public String toString() {
		...
		if (this.getDocument() != null && this.getDocument().getPatientName() != null) {
			str += "(" + this.getDocument().getPatientName() + ")";
		}
		str += "<" + Annotation.getShortenedPercentString(this.getGoodness(), 4) + ">";

		// 3/12/2016
		if (this.isInterpreted()) {
			str += "<Properties:" + this.getSemanticInterpretation().getAllPropertyString() + ">";
		}
	...
	public String toExpandedString() {
		...
		if (this.getSemanticInterpretation() != null) {
			String pstr = this.getSemanticInterpretation().getCommaDelimitedPredString();
			if (pstr != null) {
				str += "[Properties: " + pstr + "]";
			} else if (this.isInterpreted()) {
				str += "[cui=" + this.getCui() + ",concept=" + this.getConcept() + "]";
			}
			if (this.getSemanticInterpretation().getRelationSentences() != null) {
				Vector<RelationSentence> rsv = this.getSemanticInterpretation().getRelationSentences();
	...
	// 6/11/2017
	private Vector<String> dontUseRelationNames = VUtils.arrayToVector(new String[] { "concept", "type", "ruleid" });

	public String toIEVizWorkflowString() {
		String str = "";
		if (this.getSemanticInterpretation() != null) {
			Interpretation interp = this.getSemanticInterpretation();
			str = this.getText() + "|";
			str += this.getTextStart() + "-" + this.getTextEnd() + "|";
			str += (this.getType() != null ? this.getType() : "*") + "|";
			str += (this.getConcept() != null ? this.getConcept() : "*") + "|";
			String pstr = "*";
			if (interp.getPropertyNames() != null) {
				pstr = "";
				for (String pname : interp.getPropertyNames()) {
					if (!dontUseRelationNames.contains(pname)) {
						Object value = interp.getProperty(pname);
						String vstr = null;
						if (value instanceof Annotation) {
							Annotation pa = (Annotation) value;
							vstr = pa.getConcept().toString();
						} else {
							vstr = value.toString();
						}
						pstr = pname + "=" + vstr;
			...
			if (interp.getRelationSentences() != null) {
				Hashtable<String, String> rshash = new Hashtable();
				for (RelationSentence rs : this.getRelationSentences(false)) {
					if (!dontUseRelationNames.contains(rs.getRelation().getName().toLowerCase())) {
						String rstr = rs.getRelation().getName() + "(" + rs.getSubject().getConcept() + ","
								+ rs.getModifier().getConcept() + ")";
						if (rshash.get(rstr) == null) {
							rshash.put(rstr, rstr);
							str += rstr + ",";
						}
					}
				}
	...
	// 7/20/2017
	public String getTypeConceptDisplayString() {
		String str = null;
		String typename = "undefined-rule-type";
		if (this.getType() != null) {
			typename = this.getType().getName();
		}
		if (this.getConcept() != null) {
			if (typename != null) {
				str = "<" + typename + "=" + this.getConcept() + ">";
			} else {
				str = "<" + "NOTYPE" + "=" + this.getConcept() + ">";
			}
		}
	...
	public String getText_BEFORE_11_13_2016() {
		if (this.string != null) {
			StringBuffer sb = new StringBuffer();
			for (int i = 0; i < this.string.length(); i++) {
				char c = this.string.charAt(i);
	...
	// 11/1/2016
	public static Vector<Annotation> getNonNestedTargetDocumentAnnotations(Vector<Annotation> annotations) {
	...
	public static Vector<Annotation> getNonNestedNonCoincidingAnnotations(Vector<Annotation> annotations,
	...
	public static Vector<Annotation> getNonNestedAnnotations(Vector<Annotation> annotations) {
		...
		if (annotations != null) {
			for (Annotation annotation : annotations) {
				if (annotation instanceof StructureAnnotation) {
					...
					// 4/25/2018
					else if (annotation.isValid() && annotation.isInterpreted()) {
						ancestors = VUtils.add(ancestors, annotation);
						annotation.setVisited(true);
					}
				}
			}
			if (ancestors != null) {
	...
	// within the
	// decision list.
	public static Vector<Annotation> getNonCoincidingAnnotations(Vector<Annotation> annotations,
			boolean interpretedOnly) {
		if (annotations == null) {
			return null;
		}
		...
		for (int i = 0; i < annotations.size() - 1; i++) {
			...
			for (int j = i + 1; j < annotations.size(); j++) {
				...
				if (eitherCointainsOrCoincides(a1, a2)) {
					if (a1.getMoonstoneRuleInterface().isCompareAnnotationsTargetsAndGoodness()) {
	...
	private static Annotation selectRemovableGoodnessOnly(Annotation a1, Annotation a2) {
		Annotation remove = null;
	...
	// 3/10/2018: HUGE CHANGE. ALLOWS LARGER ANNOTATION TO GET TOSSED.
	private static Annotation selectRemovableTargetPlusGoodness(Annotation a1, Annotation a2) {
		Annotation remove = null;
		...
		// else if (a1.getNestedTargetConceptCount() >
		// a2.getNestedTargetAnnotationCount()) {
		// remove = a2;
		// } else if (a2.getNestedTargetConceptCount() >
		// a1.getNestedTargetAnnotationCount()) {
		// remove = a1;
		// }
		...
		// else if (annotationContains(a1, a2)) {
		// remove = a2;
		// } else if (annotationContains(a2, a1)) {
		// remove = a1;
		// }
		if (remove != null && remove.getConcept() != null
				&& remove.getConcept().toString().toLowerCase().contains("support")) {
			int x = 1;
		}
		return remove;
	}

	private static Annotation selectRemovableTargetPlusGoodness_BEFORE_3_10_2018(Annotation a1, Annotation a2) {
		Annotation remove = null;
		if (a1.containsTargetConcept() && !a2.containsTargetConcept()) {
			remove = a2;
		} else if (a2.containsTargetConcept() && !a1.containsTargetConcept()) {
	...
	public static Vector<Annotation> getParentlessAnnotations(Vector<Annotation> annotations) {
		Vector<Annotation> parentless = null;
		if (annotations != null) {
			for (Annotation annotation : annotations) {
				if (annotation.getParents() == null) {
	...
	public Vector<Annotation> getNonNestedAncestors() {
		...
		for (Term t : this.getParents()) {
			Annotation parent = (Annotation) t;
			if (parent.isValid()) {
				ancestors = VUtils.append(ancestors, parent.getNonNestedAncestors());
			}
		}
		return ancestors;
	}
	...
	public static Vector<Annotation> getNonNegativeNestedTargetAnnotations(MoonstoneRuleInterface msri,
			...
			for (Annotation annotation : annotations) {
				...
				if (nonnegs == null) {
					...
					for (Annotation nonneg : nonnegs) {
						...
						if (msri.getReadmission().conceptsAreNegative(nnstr, astr)) {
							found = true;
							break;
						}
					}
	...
	public void gatherNestedTargetAnnotations() {
		...
		// 3/1/2018: Permitting negated target concepts in R01
		// if (this.containsTargetConcept() && !neg) {
		...
		if (this.hasChildren()) {
			for (Annotation child : this.getChildAnnotations()) {
				if (child.nestedTargetAnnotations != null
						&& !(child.getRule() != null && child.getRule().isCaptureRule())) {
					for (Annotation cna : child.nestedTargetAnnotations) {
						Object cc = cna.getConcept();
						if (!(neg && concept.equals(cc))) {
							this.nestedTargetConcepts = VUtils.addIfNot(this.nestedTargetConcepts, cc);
							this.nestedTargetAnnotations = VUtils.add(this.nestedTargetAnnotations, cna);
						}
					}
				}
			}
	...
	// 6/21/2017: Don't add child's topmost nested annotation if negated.
	public void gatherTopmostNestedTargetAnnotations() {
		...
		if (this.containsTargetConcept() && !neg) {
			...
			for (Annotation child : this.getChildAnnotations()) {
				if (child.topmostNestedTargetAnnotations != null && !child.getRule().isCaptureRule()) {
					for (Annotation ta : child.topmostNestedTargetAnnotations) {
						if (!(neg && concept.equals(ta.getConcept()))) {
							this.topmostNestedTargetAnnotations = VUtils.add(this.topmostNestedTargetAnnotations, ta);
						}
					}
				}
			}
	...
	public static class ConceptCommaSorter implements Comparator {
		public int compare(Object o1, Object o2) {
			Annotation a1 = (Annotation) o1;
			Annotation a2 = (Annotation) o2;
			int c1 = (a1.getConcept() instanceof StringConstant ? ((StringConstant) a1.getConcept()).getNumberOfColons()
					: 100);
			int c2 = (a2.getConcept() instanceof StringConstant ? ((StringConstant) a2.getConcept()).getNumberOfColons()
					: 100);
	...
	public static class NumericIDSorter implements Comparator {
		public int compare(Object o1, Object o2) {
			Annotation a1 = (Annotation) o1;
			Annotation a2 = (Annotation) o2;
			long id1 = a1.getNumericAnnotationID();
			long id2 = a2.getNumericAnnotationID();
			if (id1 > id2) {
				return -1;
			} else if (id1 < id2) {
				return 1;
	...
	public boolean hasSameConcept(Annotation other) {
		return ((this.getConcept() != null && this.getConcept().equals(other.getConcept()))
				|| (this.getCui() != null && this.getCui().equals(other.getCui())));
	}
	...
	public static class TextLengthSorter implements Comparator {
		public int compare(Object o1, Object o2) {
			try {
				Annotation a1 = (Annotation) o1;
				Annotation a2 = (Annotation) o2;
				int l1 = a1.getTextLength();
				int l2 = a2.getTextlength();
				if (l1 > l2) {
					return -1;
				} else if (l2 > l1) {
					return 1;
	...
	public static class WordCountSorter implements Comparator {
		public int compare(Object o1, Object o2) {
			try {
				Annotation a1 = (Annotation) o1;
				Annotation a2 = (Annotation) o2;
				int l1 = a1.getWordTokenCount();
				int l2 = a2.getWordTokenCount();
				if (l1 > l2) {
					return -1;
				} else if (l2 > l1) {
					return 1;
	...
	public static class TextLengthSorterPlusConceptLengthSorter implements Comparator {
		public int compare(Object o1, Object o2) {
			try {
				int x = 1;
				Annotation a1 = (Annotation) o1;
				Annotation a2 = (Annotation) o2;
				int tlen1 = a1.getCoveredTextLength();
				int tlen2 = a2.getCoveredTextLength();
				if (tlen1 < tlen2) {
					return -1;
				}
				if (tlen2 < tlen1) {
				...
				int clen1 = (a1.getConcept() != null ? a1.getConcept().toString().length() : 0);
				int clen2 = (a2.getConcept() != null ? a2.getConcept().toString().length() : 0);
				if (clen1 < clen2) {
					return -1;
				}
				if (clen2 < clen1) {
	...
	public boolean before(Annotation annotation) {
		return SeqUtils.before(this.getTokenStart(), this.getTokenEnd(), annotation.getTokenStart(),
				annotation.getTokenEnd());
	}

	public boolean after(Annotation annotation) {
		return SeqUtils.after(this.getTokenStart(), this.getTokenEnd(), annotation.getTokenStart(),
				annotation.getTokenEnd());
	}
	...
	public boolean contains(Annotation annotation) {
		return !this.equals(annotation) && SeqUtils.contains(this.getTokenStart(), this.getTokenEnd(),
	...
	public boolean strictlyCovers(Annotation other) {
		return this.covers(other) && (this.tokenStart < other.tokenStart || this.tokenEnd > other.tokenEnd);
	}

	public boolean coincides(Annotation other) {
		if (!this.equals(other) && this.tokenStart == other.tokenStart && this.tokenEnd == other.tokenEnd) {
			return true;
		}
		return false;
	}
	...
	boolean isNonNestedConcept() {
		...
		boolean isCore = (hasConcept && AnnotationIntegrationMaps.getCUI(concept) != null);
		...
		if (this.getRule() != null && this.getRule().getOntology() != null && this.getRule().getOntology().isOnyx()
				&& this.hasMacro()) {
			return true;
		}
		...
		if (cp != null && cp.getType() != null && !(cp.getType().isCondition() || cp.getType().isLocation())) {
			return false;
		}
		// 3/25/2014
	...
	public WordSequenceAnnotation getSentenceAnnotation() {
		if (this.sentenceAnnotation == null && this instanceof WordSequenceAnnotation) {
	...
	public Vector<Token> getTokens() {
		...
		if (this.tokens == null && document != null && document.getTokens() != null && this.getTokenStart() >= 0) {
			this.tokens = VUtils.subVector(document.getTokens(), this.getTokenStart(), this.getTokenEnd() + 1);
		}
		return this.tokens;
	}
	...
	public boolean sameOntology(Annotation other) {
		return ((this.ontology == null && other.ontology == null)
				|| (this.ontology != null && this.ontology.equals(other.ontology)));
	}
	...
	public Annotation getSingleChild() {
		if (this.textuallySortedSourceAnnotations != null && this.getTextuallySortedSourceAnnotations().size() == 1) {
	...
	public Object getPropertyFromRuleOrSourceAnnotations(String attribute) {
		...
		if (rule != null) {
			...
			// 5/24/2018 Updated: I'm adding "(concept -complex-)" to
			// rule s-expressions in Excel.
			if ((value == null || "-complex-".equals(value)) && "concept".equals(attribute)
				...
				if (cname != null && cname.length() > 1) {
					TypeConstant root = rule.getGrammar().getGrammarModule().getKnowledgeEngine().getCurrentOntology()
							.getRootType();
					StringConstant sc = StringConstant.createStringConstant(cname, root, true);
					value = sc;
				} else {
					// 10/1/2018: For use with grammars converted to have word-level rules with
					// (concept -complex-)..
					value = null;
				}
			}

			if (value == null && this.getSourceAnnotations() != null && rule.isInheritProperties()
	...
	public void generateIndexTokens() {
		Rule rule = this.getRule();
		this.indexTokens = null; // 9/12/2018
		...
		TypeConstant sctype = null;
		if (concept instanceof StringConstant) {
			sc = (StringConstant) concept;
			sctype = sc.getType();
		}

		// 3/13/2018
		if (concept != null) {
			if (!(concept instanceof Term) || ((Term) concept).isUsedInGrammarRulePattern()) {
		...
		if (concept != null && ":POSSIBLE-SUPPORT:".equals(concept.toString())) {
			int x = 1;
		}

		// Before 3/13/2018
		// if (this.isTerminal() || (concept instanceof Constant)) {
		// if (concept != null && !((Term) concept).isUsedInGrammarRulePattern()) {
		// int x = 1;
		// }
		// this.indexTokens = VUtils.add(this.indexTokens, concept);
		// }
		...
		if (this.getType() != null) {
			...
			if (type.getUnifiableTypes() == null) {
				if (type.isUsedInGrammarRulePattern()) {
					this.indexTokens = VUtils.add(this.indexTokens, type);
				}

				// 8/10/2016
				// REMOVED 9/12/2017- I'M NOT CONSISTENTLY USING PHRASE TYPES YET...
				// if (this.getPhraseType() != null) {
				// this.indexTokens = VUtils.add(this.indexTokens,
				// Annotation.getPhrasePlusTypeIndex(this.getPhraseType(), this.getType()));
				// }

			} else {
				for (TypeConstant unifier : this.getType().getUnifiableTypes()) {
					if (!unifier.isRoot()) {
						if (unifier.isUsedInGrammarRulePattern()) {
							this.indexTokens = VUtils.add(this.indexTokens, unifier);
						}

						// 8/10/2016
						if (this.getPhraseType() != null) {
							String pti = Annotation.getPhrasePlusTypeIndex(this.getPhraseType(), unifier);
							// this.indexTokens = VUtils.add(this.indexTokens, pti);
						}
					}
				}
			}
		...
		// 3/1/2018: The annotation type may come from the grammar rule. Also need to
		// add
		// types from the concept.
		if (sctype != null && !sctype.equals(this.getType()) && sctype.getUnifiableTypes() != null) {
			for (TypeConstant unifier : sctype.getUnifiableTypes()) {
				if (!unifier.isRoot() && !unifier.isUndefined()) {
					if (unifier.isUsedInGrammarRulePattern()) {
						this.indexTokens = VUtils.add(this.indexTokens, unifier);
					}
				}
		...
		// 9/13/2017: Add constant + basetype strings, for use
		// with the syntagmatic ontologies
		if (sctype != null && sctype.getNumberOfBaseTypes() > 1) {
			for (TypeConstant basetype : sctype.getBaseTypes()) {
				String str = getConceptPlusBaseTypeIndex(sc, basetype);
				if (rule != null && rule.getGrammar().hasRulesByIndexToken(str)) {
					this.indexTokens = VUtils.add(this.indexTokens, str);
				}
			}
		}
		...
		if (this.getPhraseType() != null) {
			this.indexTokens = VUtils.add(this.indexTokens, this.getPhraseType());
		}
		...
		if (rule != null && rule.isDoDebug()) {
			Vector tokens = this.indexTokens;
			int x = 1;
		}
	...
	public String getPhrasePlusTypeIndex() {
		if (this.getPhraseType() != null && this.getType() != null) {
			String str = this.getPhraseType().getFormalName() + "=" + this.getType().getFormalName();
			return str;
		}
		return null;
	}

	public static String getPhrasePlusTypeIndex(SyntacticTypeConstant ptc, TypeConstant tc) {
		if (ptc != null && tc != null) {
	...
	// 9/13/2017
	public static String getConceptPlusBaseTypeIndex(StringConstant concept, TypeConstant basetype) {
		String str = null;
		int x = 1;
		if (basetype != null) {
			Vector<TypeConstant> path = concept.getType().getPathToBaseType(basetype);
			boolean isbase = (concept.getType().getPathToBaseType(basetype) != null);
			if (!isbase) {
				basetype = basetype.getFirstBaseType();
			}
			str = concept.getFormalName() + basetype.getFormalName();
		}
		return str;
	}
	...
	public CUIStructureShort getCuiStruct() {
		return UMLSStructuresShort.getUMLSStructures().getCUIStructure(this.getCui());
	}
	...
	public void setSignature_MINIMAL_5_10_2018() {
		if (this.signature == null) {
			...
			String conceptstr = (this.getConcept() != null ? this.getConcept().toString() : "*");
	...
	public void setSignature() {
		if (this.signature == null) {
			...
			String rulestr = (this.getRule() != null ? String.valueOf(this.getRule().getRuleIDNum()) : "*");
			sb.append(rulestr + "^");
			String conceptstr = (this.getConcept() != null ? this.getConcept().toString() : "*");
			sb.append(conceptstr + "^");

			// 5/10/2018- Try not using child details
			boolean useChildren = false;
			if (useChildren && this.hasChildren()) {
				sb.append("(");
				for (int i = 0; i < this.getNumberOfChildren(); i++) {
					Annotation child = this.getLexicalChild(i);
					String cstr = "[";
					String ccstr = (child.getConcept() != null ? child.getConcept().toString() : "*");
					String crstr = (child.getRule() != null ? String.valueOf(child.getRule().getRuleIDNum()) : "*");
					cstr += "[" + crstr + "^" + ccstr + "]";
					// String cstartend = "[" + child.getTokenStart() + "-" + child.getTokenEnd() +
					// "]";
					// cstr += "[" + ccstr + "^" + crstr + "^" + cstartend + "]";
					sb.append(cstr);
				}
				sb.append(")");
			}
	...
	public void setSignature_BEFORE_4_30_2018() {
		if (this.signature == null) {
			this.signatureID = LastSignatureID++;
			StringBuffer sb = new StringBuffer();
			sb.append("[");
			...
			String typestr = (this.getType() != null ? String.valueOf(this.getType().getNumericID()) : "*");
			sb.append(typestr + "^");

			String ptypestr = (this.getPhraseType() != null ? String.valueOf(this.getPhraseType().getNumericID())
					: "*");
			sb.append(ptypestr + "^");
				...
				for (int i = 0; i < this.getNumberOfChildren(); i++) {
					...
					if (child.getType() != null || child.getRule() != null) {
						...
						if (child.getType() != null) {
							String cts = String.valueOf(child.getType().getNumericID());
							cstr += cts + "-";
						}
						if (child.getRule() != null) {
							String crs = String.valueOf(child.getRule().getRuleIDNum());
							cstr += crs;
						}
						cstr += "]";
						sb.append(cstr);
	...
	protected boolean typeLoopCheck(Annotation parent) {
		Rule rule = this.rule;
		if (this.isSingleton()) {
			Annotation child = this.textuallySortedSourceAnnotations.firstElement();
			if (parent.getRule() == child.getRule() && parent.getConcept().equals(child.getConcept())) {
				return true;
			}
			// If there is just one child, and the type is specialized visavis
			// the component
			// type, then it needs to add specialization via tests.
			if (this.getType() != null && child.getType() != null && this.getType().isSubsumedBy(child.getType())
					&& !this.containsTests()) {
				return true;
			}
	...
	protected boolean containsTests() {
		Rule rule = this.getRule();
		if (rule != null && (rule.getLocalVariables() != null || rule.getTestPredicates() != null
	...
	protected boolean isSingleton() {
		return this.textuallySortedSourceAnnotations != null && this.textuallySortedSourceAnnotations.size() == 1;
	}

	// 12/7/2014: For use from outside parser, e.g. TSL
	public static boolean intervalContainsStrings(Annotation a1, Annotation a2, Vector<String> strs) {
		if (a1 != null && a2 != null && a1.getSentenceAnnotation() == a2.getSentenceAnnotation() && strs != null) {
			Vector<Token> tokens = a1.getSentenceAnnotation().getSentence().getTokens();
			Annotation first = (a1.getTextStart() < a2.getTextStart() ? a1 : a2);
			Annotation second = (a1 == first ? a2 : a1);
			if (first.getTokenEnd() < second.getTokenStart()) {
				for (int i = first.getRelativeTokenEnd() + 1; i < second.getRelativeTokenStart() - 1; i++) {
	...
	public Vector<String> getParentTypeIndices() {
		return getAllParentTypeIndices(this.getSemanticInterpretation().getGeneralType());
	}
	...
	public static Vector<String> getAllParentTypeIndices(TypeConstant type) {
		...
		if (type != null) {
			...
			if (type.getParents() != null) {
				TypeConstant ptype = (TypeConstant) type.getParents().firstElement();
				indices = VUtils.append(indices, getAllParentTypeIndices(ptype));
			}
		}
		return indices;
	}
	...
	public TSLInformation getPropertySource() {
		return (this.getSemanticInterpretation() != null ? this.getSemanticInterpretation() : this);
	}
	...
	public Vector<RelationSentence> getRelationSentences(boolean useInference) {
		...
		if (this.getSemanticInterpretation() != null) {
			rsents = (useInference ? this.getSemanticInterpretation().getAllRelationSentences()
	...
	public static Vector<RelationSentence> getRelationSentences(Vector<Annotation> annotations) {
		Vector<RelationSentence> rsents = null;
		if (annotations != null) {
			for (Annotation annotation : annotations) {
				if (annotation.getSemanticInterpretation() != null) {
					Vector<RelationSentence> rs = annotation.getSemanticInterpretation().getRelationSentences();
	...
	public boolean isSourceLearned() {
		return this.getSemanticInterpretation() != null && this.getSemanticInterpretation().isSourceLearned();
	}
	...
	public boolean containsRuleFlag(String flag) {
		return this instanceof TagAnnotation && this.getRule() != null && this.getRule().containsFlag(flag);
	}
	...
	public float getLeftDepth() {
		if (this.leftDepth == 0 && this.textuallySortedSourceAnnotations != null) {
			this.leftDepth = this.textuallySortedSourceAnnotations.firstElement().getLeftDepth() + 1;
		}
		return this.leftDepth;
	}

	public float getRightDepth() {
		if (this.rightDepth == 0 && this.textuallySortedSourceAnnotations != null) {
			this.rightDepth = this.textuallySortedSourceAnnotations.lastElement().getRightDepth() + 1;
		}
	...
	public boolean testHeader() {
		Rule rule = this.getRule();
		if (rule != null && rule.getHeaderStrings() != null && this.getSentenceAnnotation() != null
				&& this.getSentenceAnnotation().getSentence() != null) {
			Header header = this.getSentenceAnnotation().getSentence().getHeader();
	...
	public boolean isWord() {
		return this.getTokenLength() == 1 && this.getTokens().firstElement().isWord();
	}
	...
	public boolean isPossessiveWord() {
		if (this.isWord()) {
			...
			return len > 4 && str.charAt(len - 1) == '\'' && str.charAt(len - 1) == 's';
		}
		return false;
	}
	...
	public boolean hasProperty(String attribute) {
		return getProperty(attribute) != null;
	}
	...
	public boolean validateRuleSubpattern() {
		...
		if (subpatternLists == null || singleton == null || singleton.getNumberOfChildren() != subpatternLists.size()) {
			return true;
		}
		if (rule != null && rule.isDoDebug() && rule.getSubpatternLists() != null) {
			int x = 1;
		}
	...
	public static Vector<String> IrrelevantPropertyNames = VUtils
			.arrayToVector(new String[] { "ruleid", "concept", "cui", "macro", "type" });
	...
	public String toHTML() {
		...
		if (si != null) {
			...
			str += "Offset=" + this.getTextStart() + "-" + this.getTextEnd() + "<br>";
			if (this.getRule() != null) {
				str += "Rule=" + this.getRule().getRuleID() + "(" + this.getRule().getSourceFilePath() + ")" + "<br>";
			}
			if (si.getType() != null) {
				// str += "semanticType=" + si.getType().getName() + "<br>";
				str += "semanticType=" + si.getType().getRightmostFullNamePath() + "<br>";
			}
			if (this.getRule() != null && this.getRule().getPhraseType() != null) {
				str += "phraseType=" + this.getRule().getPhraseType().getName() + "<br>";
			}
			if (si.getConcept() != null) {
				str += "concept=" + si.getConcept() + "<br>";
			}
			if (this.FCIETargetRuleName != null) {
				str += "&nbsp;&nbsp;&nbsp;&nbsp;**Deduced target from rule=" + this.FCIETargetRuleName + "**<br>";
			}
			str += "CoveredTargetConcepts=" + this.getNestedTargetConceptString() + "<br>";
			...
			for (String property : si.getPropertyNames()) {
				if (PropertyIsRelevant(property)) {
					Object value = this.getProperty(property);
					if (!(value instanceof Annotation)) {
						str += "&nbsp;&nbsp;&nbsp;&nbsp;" + property + "=" + value.toString() + "<br>";
					}
				}
			}
			if (si.getRelationSentences() != null) {
				...
				for (RelationSentence rs : si.getRelationSentences()) {
					...
					if (rs.getSubject() instanceof Annotation && rs.getModifier() instanceof Annotation) {
						Annotation subject = (Annotation) rs.getSubject();
						Annotation modifier = (Annotation) rs.getModifier();
						String sstr = (subject == this ? "?*" : "\"" + subject.getText() + "\"");
						String mstr = "\"" + modifier.getText() + "\"";
						String rsstr = "(" + rs.getRelation().toString() + " " + sstr + " " + mstr + ")";
						str += "&nbsp;&nbsp;&nbsp;&nbsp;" + rsstr + "<br>";
					}
				}
			}
			str += "Goodness=" + getShortenedPercentString(this.getGoodness(), 4) + "<br>";
			str += "LCPFG=" + getShortenedPercentString(this.getLPCFGProbability(), 4) + "<br>";
			str += "Coverage=" + getShortenedPercentString(this.getCoveredTextPercent(), 4) + "<br>";
			str += "SymmetryFactor=" + getShortenedPercentString(this.getSymmetryFactor(), 4) + "<br>";
			str += "TokenIndices=" + this.getIndexTokens().toString() + "<br>";
			str += "Signature=" + this.getSignature() + "<br>";
			str += "Header=" + this.getSentenceAnnotation().getSentence().getHeader().getText() + "\n";
			if (!this.isValid()) {
	...
	public String toDescription(int spaces) {
		...
		if (si != null) {
			...
			str += getSpaces(spaces) + "Offset=" + this.getTextStart() + "-" + this.getTextEnd() + "\n";
			if (this.getRule() != null) {
			...
			for (String property : si.getPropertyNames()) {
				if (PropertyIsRelevant(property)) {
					Object value = this.getProperty(property);
					if (!(value instanceof Annotation)) {
						str += getSpaces(spaces) + "    " + property + "=" + value.toString() + "\n";
					}
				}
			}
			if (si.getRelationSentences() != null) {
			...
			str += getSpaces(spaces) + "Goodness=" + getShortenedPercentString(this.getGoodness(), 4) + "\n";
			str += "Header=" + this.getSentenceAnnotation().getSentence().getHeader().getText() + "\n";
		}
		str += "\n";
		return str;
	...
	public int getCoveredTextLength() {
		if (this.coveredTextLength == 0) {
			if (this.getChildAnnotations() != null) {
				...
				this.coveredTextLength = (this.getTextEnd() - this.getTextStart()) + 1;
			}
			this.coveredTextPercent = (double) this.coveredTextLength / (double) this.textlength;
		}
	...
	public String toXML() {
		...
		sb.append("<position>" + this.getTextStart() + "-" + this.getTextEnd() + "</position>\n");
		String type = (this.getType() != null ? this.getType().getFormalName() : "*");
		String concept = (this.getConcept() != null ? this.getConcept().toString() : "*");
		sb.append("<concept>" + concept + "</concept>\n");
		sb.append("<type>" + type + "</type>\n");
		if (this.getRule() != null) {
			sb.append("<ruleID>" + this.getRule().getRuleID() + "</ruleID>\n");
		}
		if (this.isInterpreted() && this.getSemanticInterpretation().getPropertyNames() != null) {
			sb.append("<properties>\n");
			Hashtable props = this.getAllProperties();
			for (Enumeration<String> e = props.keys(); e.hasMoreElements();) {
				String property = e.nextElement();
				if (!VUtils.containedIn(property, new String[] { "ruleid", "concept", "macro", "contains-target" })) {
					...
					sb.append("<property=\"" + property + "\">" + vstr + "</property>\n");
				}
			}
			sb.append("</properties>\n");
			...
			if (rule != null && rule.getSemanticRelations() != null) {
				sb.append("<relations>\n");
				for (PatternRelationSentence prs : this.getRule().getSemanticRelations()) {
					String rname = prs.getRelation().getName();
					Object o1 = this.evalPattern(prs.getSubject(), this.getVariables());
					Object o2 = this.evalPattern(prs.getModifier(), this.getVariables());
					if (o1 instanceof Annotation && o2 instanceof Annotation) {
						...
						if (subject != null && modifier != null) {
							...
							sb.append("<subject=\"" + subject.getAnnotationIDOrText() + "\">\n");
							sb.append("<modifier=\"" + modifier.getAnnotationIDOrText() + "\">\n");
							sb.append("<relation>\n");
						}
					}
				}
	...
	public static Vector<Annotation> getNonNestedAnnotationsWithoutDuplicateConcepts(Vector<Annotation> annotations) {
		...
		if (v != null) {
			for (Annotation annotation : v) {
				...
				if (concept != null) {
					...
					if (other == null || other.getGoodness() < annotation.getGoodness()) {
	...
	public static Vector<String> getAllConceptStrings(Vector<Annotation> annotations) {
		Vector<String> concepts = null;
	...
	public static Vector<Annotation> getTargetAnnotations(Vector<Annotation> annotations) {
		Vector<Annotation> targets = null;
		if (annotations != null) {
			for (Annotation annotation : annotations) {
				if (annotation.containsTargetConcept()) {
	...
	public static Vector<Annotation> removeSameOffsetDuplicates(Vector<Annotation> annotations) {
		Annotation last = null;
		Vector<Annotation> nodups = null;
		for (Annotation annotation : annotations) {
			boolean isdup = false;
			if (last != null && last.getTextStart() == annotation.getTextStart()
	...
	private static String getParsetreePathName(Annotation ancestor, Annotation descendent, int offset) {
		...
		if (ancestor.hasChildren()) {
			for (int i = 0; i < ancestor.getNumberOfChildren(); i++) {
				Annotation child = ancestor.getLexicallySortedSourceAnnotations().elementAt(i);
	...
	public boolean hasImproperSemanticSegmentation() {
		...
		if (this.isInterpreted() && this.getNumberOfChildren() == 2) {
			...
			if (cphrase1.getType() != null && !cphrase1.getType().equals(cphrase2.getType()) && cphrase1.hasChildren()
					&& cphrase2.hasChildren()) {
				Annotation ccphrase1 = cphrase1.getChildAnnotations().lastElement();
				Annotation ccphrase2 = cphrase2.getChildAnnotations().firstElement();
				if (ccphrase1.getType() != null && ccphrase1.getType().equals(ccphrase2.getType())) {
					return true;
				}
	...
	public static Vector<Annotation> flatten(Vector<Annotation> annotations) {
		...
		if (annotations != null) {
			for (Annotation annotation : annotations) {
				...
				all = VUtils.append(all, flatten(annotation.getChildAnnotations()));
			}
		}
		return all;
	}
	...
	public double getGoodness() {
		try {
			if (this.goodness == null) {
				...
				double sf = this.symmetryFactor = conjunctSymmetry * childSymmetryFactorProduct;
				double ctp = this.getCoveredTextPercent();
				double formresult = ctp * sf;

				double pcfgprob = (this.getMoonstoneRuleInterface().getLpcfg()
						.isUseRuleConditionalsOnlyInProbabilityCalculation() ? this.getPCFGProbability()
								: this.getLPCFGProbability());

				double specfactor = (this.hasRule() && this.getRule().isSpecialized() ? 1 : 0);
				double abstractfactor = (this.hasRule() && this.getRule().isComplexConcept() ? 1 : 0);
				double pvalue = (formresult * formweight) + (pcfgprob * pcfgweight) + (specfactor * specweight)
	...
	public double getPCFGProbability() {
		if (this.pcfgProbability == null) {
			...
			if (this.grammar.getGrammarModule().getMoonstoneRuleInterface().getLpcfg() != null) {
				this.grammar.getGrammarModule().getMoonstoneRuleInterface().getLpcfg().calculatePCFGProbability(this);
			}
		}
	...
	public double getLPCFGProbability() {
		if (this.lpcfgProbability == null) {
			...
			if (this.grammar.getGrammarModule().getMoonstoneRuleInterface().getLpcfg() != null) {
				this.grammar.getGrammarModule().getMoonstoneRuleInterface().getLpcfg().calculateLPCFGProbability(this);
			}
		}
	...
	public boolean isNumber() {
		if (this.getConcept() != null && ":NUMBER:".equals(this.getConcept().toString())) {
			return true;
		}
		return false;
	}
	...
	public boolean isValidPerTrainingOffsets() {
		Vector<int[]> offsets = this.getGrammar().getGrammarModule().getMoonstoneRuleInterface()
				.getAnnotationAttachments();
		if (offsets.size() > 0) {
			for (int[] t : offsets) {
				int tstart = this.getTextStart();
				int tend = this.getTextEnd();
				if (SeqUtils.overlaps(tstart, tend, t[0], t[1]) && !SeqUtils.contains(tstart, tend, t[0], t[1])
						&& (tstart < t[0] || tend > t[1])) {
	...
	public boolean isTypeSubsumedBy(TypeConstant type) {
		return type != null && this.getType() != null && this.getType().isSubsumedBy(type);
	}

	private static double SignificantAnnotationDifference = 0.05;

	// 11/12/2016
	protected static Annotation annotationComparisonDecisionList(Annotation a1, Annotation a2) {
		...
		// 7/18/2016
		if (a1 instanceof DocumentAnnotation && a2 instanceof NarrativeAnnotation) {
			return a1;
		}
		if (a2 instanceof DocumentAnnotation && a1 instanceof NarrativeAnnotation) {
			return a2;
		}

		boolean a1specialized = (a1.getRule() != null && a1.getRule().isSpecialized());
		boolean a2specialized = (a2.getRule() != null && a2.getRule().isSpecialized());
		...
		if (msri.isCompareAnnotationsTargetsAndGoodness()) {
			...
			// 12/18/2017
			if (a1.containsTargetConcept() && a2.containsTargetConcept() && !a1.getConcept().equals(a2.getConcept())) {
				return null;
			}
			...
			int totalTarget1 = a1.getNestedTargetAnnotationCount();
			int totalTarget2 = a2.getNestedTargetAnnotationCount();
		...
		if (r1 != null && r2 != null) {
			...
			// 4/2/2018: Keep shorter complex concepts: Does this remove valid competitors?
			if (r1.isComplexConcept() && r2.isComplexConcept()) {
				StringConstant sc1 = (StringConstant) a1.getConcept();
				StringConstant sc2 = (StringConstant) a2.getConcept();

				if (sc1 != null || sc2 != null) {
					if (sc1.getNumberOfColons() < sc2.getNumberOfColons()) {
						// return a1;
					} else if (sc1.getNumberOfColons() > sc2.getNumberOfColons()) {
						// return a2;
					}
				}
			}
		...
		if (a1.getType() != a2.getType() && a1.getType() != null && a2.getType() != null) {
		...
		// 3/10/2018 TEST
		if (a1.getConcept() != null && a1.getConcept().equals(a2.getConcept())) {
			return (a1.getNumericAnnotationID() > a2.getNumericAnnotationID() ? a1 : a2);
		}

		return a1;
	}

	// 11/12/2016: Before I allowed phrases where one includes the other.
	protected static Annotation annotationComparisonDecisionList_BEFORE_11_12_2016(Annotation a1, Annotation a2) {
		int x = 1;
		...
		if (totalTarget1 > 0 || totalTarget2 > 0) {
			...
			// 11/1/2016 -- This seems too obvious. Why didn't I do it before?
			if (totalTarget1 > totalTarget2) {
				return a1;
			}
			if (totalTarget1 < totalTarget2) {
	...
	// annotationComparisonDecisionList_BEFORE_11_1_2016(
	...
	public void setInferredTargetConcept() {
		int x = 1;
		if (this.getRule() != null && this.isInterpreted() && !this.containsTargetConcept() && rule.isComplexConcept()
				&& !this.getSemanticInterpretation().isInvokedInference()
				&& this.getMoonstoneRuleInterface().isUseFCIEToInferTargetConcept()) {
			Vector<RelationSentence> isents = this.getSemanticInterpretation().getInferredRelationSentences();
			if (isents != null) {
				for (RelationSentence isent : isents) {
					RelationConstant rc = isent.getRelation();
					if (GrammarEBL.InferredTargetRelationName.equals(rc.getName())) {
						Object concept = isent.getModifier();
						if (concept != null) {
							this.setConcept(concept);
							this.getSemanticInterpretation().setConcept(concept);
							// Note: The "concept" RelSent is unchanged..
							this.setProperty("contains-target", true);
							this.indexTokens = VUtils.add(this.indexTokens, concept);
							this.FCIETargetRuleName = isent.getDerivedFrom().getNamedSentence().getName();
							System.out.println("Derived from: " + isent.getDerivedFrom().getNamedSentence().getName());
							return;
						}
					}
				}
	...
	public boolean isSentenceLength() {
		...
		int ssize = this.getSentenceAnnotation().getSentence().getTokens().size();
	...
	public MoonstoneRuleInterface getMoonstoneRuleInterface() {
		if (this.getGrammar() != null) {
			return this.getGrammar().getGrammarModule().getMoonstoneRuleInterface();
		}
		return null;
	}
	...
	public static boolean annotationContains(Annotation a1, Annotation a2) {
		return SeqUtils.simpleContains(a1.getTokenStart(), a1.getTokenEnd(), a2.getTokenStart(), a2.getTokenEnd());
	}

	public static boolean annotationsCoincide(Annotation a1, Annotation a2) {
		return SeqUtils.coincides(a1.getTokenStart(), a1.getTokenEnd(), a2.getTokenStart(), a2.getTokenEnd());
	}

	public static boolean eitherCointainsOrCoincides(Annotation a1, Annotation a2) {
		return annotationContains(a1, a2) || annotationContains(a2, a1) || annotationsCoincide(a1, a2);
	}

	// 8/26/2017: For use with MySQL
	public int getAnnotationDBID() {
		return annotationDBID;
	}

	public void setAnnotationDBID(int annotationDBID) {
		this.annotationDBID = annotationDBID;
	}
	...
	// 10/25/2017: Used with ConText-based parse flow
	public void invalidateDerivativeAnnotations(String reason) {
		if (this.getParents() != null) {
			for (Term pt : this.getParents()) {
				Annotation parent = (Annotation) pt;
				parent.setInvalidReason(reason);
				parent.setValid(false);
				parent.invalidateDerivativeAnnotations(reason);
			}
		}
	}

	// 10/27/2017: Create conjunct with modifiers distributed over children. Need to
	// find more general way to
	public void createConjunctWithDistributedModifiers(Rule rule) {
		Vector<Annotation> newchildren = new Vector(0);
		for (Annotation child : this.getChildAnnotations()) {
			Annotation newchild = new Annotation(child);
			JavaFunctions.applyModifierProperties(rule, newchild);
			newchildren.add(newchild);
		}
		new Annotation(this.getSentenceAnnotation(), this.getRule(), newchildren, true);
	}
	...
	////////////////////////////////////////////////////////////////////////
	// 12/7/2017: IMPORTING READMISSION FUNCTIONALITY TO EXTRACT TOPMOST RELEVANT
	...
	private Vector<Annotation> gatherNonconflictingNestedRelevantAnnotations(Hashtable chash) {
		Vector<Annotation> annotations = null;
		Readmission readmission = this.getMoonstoneRuleInterface().getReadmission();
		Rule rule = this.getRule();
		DocumentGrammar dgrammar = (DocumentGrammar) this.getMoonstoneRuleInterface().getDocumentGrammar();
		...
		// 3/1/2018: TOOK OUT NEGATION CHECKS. TARGET CONCEPTS CAN BE NEGATED. ONLY
		// CHECK
		// THAT A CONCEPT DOESN'T EXIST HIGHER IN THE TREE.
		// if (!this.isNegated() && this.hasChildren()) {
		if (this.hasChildren()) {
			for (Annotation child : this.getChildAnnotations()) {
				annotations = VUtils.append(annotations, child.gatherNonconflictingNestedRelevantAnnotations(chash));
			}
		}
		return annotations;
	}

	public static Vector<Annotation> gatherAnnotationsContainingTargetConcept(Vector<Annotation> annotations) {
		Vector<Annotation> targets = null;
		if (annotations != null) {
			for (Annotation annotation : annotations) {
				if (annotation.containsTargetConcept()) {
	...
	public static Vector<Annotation> removeDuplicates(Vector<Annotation> annotations) {
		Hashtable<Annotation, Annotation> dhash = new Hashtable();
		Vector<Annotation> v = null;
		if (annotations != null) {

			Collections.sort(annotations, new Annotation.WordCountSorter());
			Collections.sort(annotations, new Annotation.NumericIDSorter());

			DocumentGrammar dg = (DocumentGrammar) annotations.firstElement().getMoonstoneRuleInterface()
					.getDocumentGrammar();
			for (Annotation a1 : annotations) {
				StringConstant c1 = (StringConstant) a1.getConcept();

				if (dhash.get(a1) != null) {
					continue;
				}

				int t1start = a1.getTextStart();
				int t1end = a1.getTextEnd();

				for (Annotation a2 : annotations) {

					// 7/19/2018
					if (dhash.get(a2) != null) {
						continue;
					}

					StringConstant c2 = (StringConstant) a2.getConcept();

					int t2start = a2.getTextStart();
					int t2end = a2.getTextEnd();

					// 4/26/2018

					if (a1.getRule().isCaptureRule() && SeqUtils.contains(t1start, t1end, t2start, t2end)) {
						dhash.put(a2, a2);
					} else if (a1 != a2 && (c1.equals(c2) || dg.conceptsAreGateEquivalents(c1, c2))
							&& SeqUtils.overlaps(t1start, t1end, t2start, t2end)) {
						int tl1 = a1.getTextlength();
						int tl2 = a2.getTextlength();

						// 3/23/2018: In "can only ambulate with a walker", "can only ambulate"
						// is being preferred sometimes.

						if (SeqUtils.simpleContains(t1start, t1end, t2start, t2end)) {
							dhash.put(a2, a2);
						} else if (SeqUtils.simpleContains(t2start, t2end, t1start, t1end)) {
							dhash.put(a1, a1);

						} else if (TypeConstant.isNullOrUndefined(a1.getType())
								&& !TypeConstant.isNullOrUndefined(a2.getType())) {
							dhash.put(a1, a1);
						} else if (TypeConstant.isNullOrUndefined(a2.getType())
								&& !TypeConstant.isNullOrUndefined(a1.getType())) {
							dhash.put(a2, a2);

							// This just mirrors the above. Why did I leave this in? Remove
							// and test it.
						} else if ((a1.getType() == null || a1.getType().isUndefined())
								&& (a2.getType() != null && !a2.getType().isUndefined())) {
							dhash.put(a1, a1);
						} else if ((a2.getType() == null || a2.getType().isUndefined())
								&& (a1.getType() != null && !a1.getType().isUndefined())) {
							dhash.put(a2, a2);
						}

						else if (a1.getNumericID() < a2.getNumericID()) {
							dhash.put(a1, a1);
			...
			for (Annotation a1 : annotations) {
				if (dhash.get(a1) == null && !a1.getRule().isCaptureRule()) {
					v = VUtils.add(v, a1);
				}
			}
		}
	...
	// 4/26/2018: If there is a capture rule annotation, remove everything beneath
	// it.
	public static Vector<Annotation> removeDuplicates_BEFORE_7_19_2018(Vector<Annotation> annotations) {
		Hashtable<Annotation, Annotation> dhash = new Hashtable();
		Vector<Annotation> v = null;
		if (annotations != null) {
			...
			for (Annotation a1 : annotations) {
				...
				if (c1.toString().toLowerCase().contains("balance")) {
					int x = 1;
				}
				if (dhash.get(a1) != null) {
					continue;
				...
				for (Annotation a2 : annotations) {
					...
					if (a1.getRule().isCaptureRule() && SeqUtils.contains(t1start, t1end, t2start, t2end)) {
						...
						if (SeqUtils.simpleContains(t1start, t1end, t2start, t2end)) {
							...
							dhash.put(a1, a1);
						} else if (a1.getType().isUndefined() && !a2.getType().isUndefined()) {
							dhash.put(a1, a1);
						} else if (a2.getType().isUndefined() && !a1.getType().isUndefined()) {
							dhash.put(a2, a2);
	...
	// 4/25/2018. Simple equality test.
	public boolean equalsAnnotation(Object o) {
		...
		if (o instanceof Annotation) {
			...
			// if (!other.getSignature().equals(this.getSignature())) {
			// return false;
			// }
			if (this.getRule() == other.getRule() && other.isInterpreted()) {
				for (int i = 0; i < this.getNumberOfChildren(); i++) {
					Annotation c1 = this.getLexicalChild(i);
					Annotation c2 = other.getLexicalChild(i);
					if (c1 != c2) {
						return false;
					}
				}
	...
	public Vector<Annotation> extractTypedAnnotations(TypeConstant type) {
		if (this.isInterpreted() && this.getType() != null && this.getType().containsBaseType(type)) {
			return VUtils.listify(this);
		} else if (this.hasChildren()) {
			Vector<Annotation> tannotations = null;
			for (Annotation child : this.getChildAnnotations()) {
				tannotations = VUtils.append(tannotations, child.extractTypedAnnotations(type));
			}
			return tannotations;
		}
		return null;
	...
	public boolean isMatchingCoreference(Annotation other) {
		if (this.isPronoun()) {
			String rtstr = (String) this.getProperty("reftype");
			String aspstr = (String) this.getProperty("aspect");
			String numstr = (String) this.getProperty("number");
			String genstr = (String) this.getProperty("gender");
			String ogenstr = (String) other.getProperty("gender");
			TypeConstant reftype = (rtstr != null ? TypeConstant.findByName(rtstr) : null);
			if (!other.getType().isSubsumedBy(reftype)) {
				return false;
			}
			if (genstr != null) {
				if (!genstr.equals(ogenstr)) {
					return false;
				}
			}
			return true;
	...
	public boolean isPronoun() {
		return this.hasProperty("pronoun");
	}
