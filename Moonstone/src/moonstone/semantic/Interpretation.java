...
public class Interpretation extends Term {
	...
	public static Interpretation create(Annotation annotation, String cui, Object concept, String macro, Rule rule,
			TypeConstant type) {
		int x = 1;
	...
	public Interpretation(Annotation annotation, String cui, Object concept, String macro, Rule rule,
			TypeConstant type) {
		this(annotation);
		...
		if (this.getAnnotation().getGrammar().isTargetConcept(this.getConcept())) {
	...
	public static void create(Annotation annotation, TypeConstant type) {
		if (annotation.getSemanticInterpretation() == null) {
			Interpretation si = new Interpretation(annotation, annotation.getCui(), annotation.getConcept(),
					annotation.getMacro(), annotation.getRule(), type);
			annotation.setSemanticInterpretation(si);
		}
	}

	public static Interpretation create(Annotation annotation, String cui, String concept, TypeConstant type) {
	...
	protected void findType() {
		if (this.type == null) {
			...
			if (type == null && rule != null && rule.getResultType() != null) {
				Object to = this.annotation.evalPatternRecursive(this.annotation.getRule().getResultType(),
			...
			if (type == null && rule != null) {
				int index = Variable.getPositionVariableIndex(this.annotation.getRule().getResultConcept());
				if (index >= 0) {
	...
	protected void findConcept() {
		if (this.getConcept() == null && annotation.getRule() != null) {
			Object co = annotation.getRule().getResultConcept();
			Object concept = this.annotation.evalPatternRecursive(co, this.annotation.getVariables());
			...
			if (concept == null) {
				int x = 2;
				Object o = annotation.getPropertyFromRuleOrSourceAnnotations("concept");
	...
	protected void findCui() {
		if (this.getCui() == null && annotation.getRule() != null) {
			String cui = null;
			String cstr = annotation.getRule().getResultCUI();
			if (Variable.isVariable(cstr)) {
				cui = (String) this.annotation.evalPattern(cstr, this.annotation.getVariables());
			} else if (cstr != null) {
			...
			if (cui == null) {
				cui = (String) annotation.getPropertyFromRuleOrSourceAnnotations("cui");
			}
			this.setCui(cui);
		}
	...
	protected void findExtendedConcept() {
		if (this.extendedConcept == null) {
			...
			if (rule != null && this.annotation.hasChildren()) {
				for (Annotation child : this.annotation.getTextuallySortedSourceAnnotations()) {
				...
				if (ecv != null) {
					...
					StringConstant sc = StringConstant.createStringConstant(str, root, true);
	...
	public static Interpretation unify(Annotation annotation, Interpretation s1, Interpretation s2) {
		...
		Vector<Word> words = VUtils.appendNew(s1.getAnnotation().getCoveredWords(),
				s2.getAnnotation().getCoveredWords());
		CUIStructureShort cp = UMLSStructuresShort.getUMLSStructures().getCoveringCuiStructure(words);
		if (cp != null) {
			sinterp = new Interpretation(annotation, cp.getCui(), cp.getWordString(false), null, null, cp.getType());
		}
		return sinterp;
	}
	...
	private void setPropertiesFromRule() {
		...
		if (rule != null) {
			...
			if (rule.getPropertyPredicates() != null) {
				for (Vector pp : rule.getPropertyPredicates()) {
					...
					Object value = this.annotation.evalPatternRecursive(pp.elementAt(1),
							this.annotation.getVariables());
		...
		if (TSLInformation.getRelevantFeatures() != null) {
			for (int i = 0; i < TSLInformation.getRelevantFeatures().length; i++) {
				...
				if (this.getProperty(attribute) == null && rule != null && !rule.isPropertyToRemove(attribute)) {
					Object value = this.annotation.getPropertyFromRuleOrSourceAnnotations(attribute);
		...
		if (rule != null && rule.hasSubpattern()) {
			Interpretation source = annotation.getLexicalChild(0).getSemanticInterpretation();
			for (String property : source.getPropertyNames()) {
				if (!rule.isPropertyToRemove(property) && this.getProperty(property) == null) {
	...
	public Vector<String> getPropertyPredicates() {
		...
		String[] noPrintProperties = new String[] { "localvar", "ruleid", "type" };
		if (pnames != null) {
	...
	public static boolean semanticInterpretationsAreUnifiable(Interpretation si1, Interpretation si2) {
		if (!(si1.getConcept() == null && si2.getConcept() == null) || si1.getConcept().equals(si2.getConcept())) {
			return false;
		}
		if (!(si1.getCUI() == null && si2.getCUI() == null) || si1.getCUI().equals(si2.getCUI())) {
			return false;
		}
		if (!(si1.getMacro() == null && si2.getMacro() == null) || si1.getMacro().equals(si2.getMacro())) {
			return false;
		}
		if (!(si1.getType() == null && si2.getType() == null) || si1.getType().equals(si2.getType())) {
			return false;
		}
	...
	public String getTuffyString() {
		...
		for (RelationSentence rsent : rsents) {
			if (this.annotation.getMoonstoneRuleInterface().isValidTuffyRelationName(rsent.getRelation().getName())) {
	...
	public Vector<RelationSentence> getRelationSentences(Vector<Annotation> annotations) {
		Vector<RelationSentence> allRelations = null;
		if (annotations != null) {
			for (Annotation annotation : annotations) {
				Vector<RelationSentence> v = annotation.getSemanticInterpretation().getRelationSentences();
	...
	public Vector<RelationSentence> getRelationSentences(boolean ruleonly) {
		if (this.relationSentences == null && !(this.annotation instanceof StructureAnnotation)) {
			...
			if (rule != null && rule.getSemanticRelations() != null) {
				for (PatternRelationSentence prs : rule.getSemanticRelations()) {
					Vector allTerms = VUtils.listify(prs.getRelation().getName());
					for (int i = 0; i < prs.getTermCount(); i++) {
						Object o = prs.getTerm(i);
						Object value = annotation.evalPattern(o, annotation.getVariables());
						if (value != null) {
			...
			if (!ruleonly) {
				for (String property : this.getPropertyNames()) {
					...
					if ("concept".equals(property) && !(value instanceof Annotation)) {
						String sid = "S_" + this.annotation.getSentenceAnnotation().getNumericID();
						Term sidterm = Term.wrapTerm(sid);
						Term pnameterm = Term.wrapTerm("P_" + pname);
						int dateIndex = this.annotation.getDocument().getAdmitDateRangeIndex();
						int fileTypeIndex = this.annotation.getDocument().getGeneralDictationType();
						Term dateterm = Term.wrapTerm(dateIndex);
						Term ftypeterm = Term.wrapTerm(fileTypeIndex);
						Vector args = VUtils.listify(this.getAnnotation(), modifier, sidterm, pnameterm, dateterm,
								ftypeterm);
						// 8/25/2016: Not sure if this is the best solution; I
						// need
						// to distinguish negated annotations in Tuffy...
						String rname = (JavaFunctions.neg(this.annotation) ? "negconcept" : "concept");
						rs = new RelationSentence(rname, args);
					} else {
						rs = new RelationSentence(property, this.getAnnotation(), modifier);
					}
					rsents = VUtils.add(rsents, rs);
				}
				if (annotation.getChildAnnotations() != null) {
					for (Annotation child : annotation.getChildAnnotations()) {
						if (child.isInterpreted()) {
							...
							// child);
							// rsents = VUtils.add(rsents, rs);
							Vector<RelationSentence> csents = child.getSemanticInterpretation().getRelationSentences();
	...
	public Vector<RelationSentence> getInferredRelationSentences() {
		if (!this.invokedInference) {
			...
			ForwardChainingInferenceEngine fcie = MoonstoneRuleInterface.RuleEditor.getForwardChainingInferenceEngine();
			if (fcie != null) {
				this.inferredRelationSentences = (Vector<RelationSentence>) fcie
						.getAllInferredRelationSentences(this.getRelationSentences());
			}
		}
	...
	public Vector<RelationSentence> getAllRelationSentences() {
		...
		return VUtils.append(this.relationSentences, this.inferredRelationSentences);
	}
	...
	public Object getInferredTargetConcept() {
		...
		if (isents != null) {
			for (RelationSentence isent : isents) {
				if (GrammarEBL.InferredTargetRelationName.equals(isent.getRelation().getName())) {
