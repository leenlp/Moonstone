...
import moonstone.grammar.RuleExpansionTimer;
...
public class ReadmissionPatientResults {
	...
	public static String[] RejectDocumentNameStrings = { "SUICIDE", "suicide", "7220_800336985906_NURSING NOTE",
			"NURSING SHIFT ASSESSMENT [DT] NSA", "NURS CARE MANAGER NOTE" };

	public ReadmissionPatientResults(ReadmissionCorpusProcessor processor, MoonstoneRuleInterface moonstone) {
	...
	public void processPatients() {
		...
		if (!this.processor.doARFF) {
			...
			if (pnames != null) {
				Collections.sort(pnames, new ReadmissionPatientResults.IntegerPatientNameSorter());

				for (String pname : pnames) {
					for (String variable : this.processor.moonstone.getReadmission().getSchema().getRelevantTypes()) {
						String attribute = this.processor.moonstone.getReadmission().getRelevantTypeAttribute(variable);
						Vector<String> evalues = this.processor.moonstone.getReadmission()
								.getTypeAttributeValues(variable);
						for (String evalue : evalues) {
							ReadmissionSummaryResult rsr = new ReadmissionSummaryResult(this, pname, variable,
									attribute, evalue);
						}
					}
				}
	...
	public void writePatientXML(String pname) {
		...
		xml += "</annotations>\n";
		String outputdirname = this.moonstone.getStartupParameters().getPropertyValue("PatientResultsOutputDirectory");
	...
	public void processFilesSingleLayer() {
		String inputdirname = this.moonstone.getStartupParameters().getPropertyValue("TextInputDirectory");
		String outputdirname = this.moonstone.getStartupParameters().getPropertyValue("ResultsOutputDirectory");
		String startstr = this.moonstone.getStartupParameters().getPropertyValue("ReadmissionFileStartIndex");
		String endstr = this.moonstone.getStartupParameters().getPropertyValue("ReadmissionFileEndIndex");
		...
		if (missing != null) {
			String fname = this.processor.moonstone.getResourceDirectoryName() + File.separatorChar + "MissingPatients_"
					+ rangestr;
			...
			for (Enumeration<Integer> e = missing.elements(); e.hasMoreElements();) {
		...
		for (int i = fileStartIndex; i < fileEndIndex; i++) {
			...
			if (doc.extractPatientNameAndDatesFromFirstLine(Readmission.DocumentDateStringFormats)) {
				...
				System.out.print("Processing file " + i + ": " + tsetname + ":" + doc.getName() + "...");
				...
				System.out.println("Done.  Elapsed: " + milli + " milliseconds");
			}
		...
		System.out
				.println("Initial pcount=" + pcount + ", processed pcount = " + this.processor.totalPatients + "\n\n");
	}

	// For validation files, single layer
	public void processFilesSingleLayerCombined() {
		int x = 1;
		String inputdirname = this.moonstone.getStartupParameters().getPropertyValue("TrainingTextInputDirectory");
		if (inputdirname == null) {
			inputdirname = this.moonstone.getStartupParameters().getPropertyValue("TextInputDirectory");
		}
		String outputdirname = this.moonstone.getStartupParameters().getPropertyValue("ResultsOutputDirectory");
		...
		Vector<File> files = FUtils.readFilesFromDirectory(inputdirname, reportConstraint);
		...
		for (File file : files) {
			...
			if (doc.extractPatientNameAndDatesFromFirstLine(Readmission.DocumentDateStringFormats)) {
				this.processor.totalFilesProcessed++;
				System.out.print("Processing file " + fcount + ": " + tsetname + ":" + doc.getName() + "...");
				long start = System.currentTimeMillis();
				String pname = doc.getPatientName();
				this.patientNameHash.put(pname, pname);
				if (this.processor.doTuffy) {
	...
	public void processFilesMultiLayer() {
		...
		for (File file : files) {
			...
			if (doc.extractPatientNameAndDatesFromFirstLine(Readmission.DocumentDateStringFormats)) {
				this.processor.totalFilesProcessed++;
				System.out.print("Processing file: " + tsetname + ":" + doc.getName() + "...");
				long start = System.currentTimeMillis();
				String pname = doc.getPatientName();
				this.patientNameHash.put(pname, pname);
				if (this.processor.doTuffy) {
	...
	private class RunMoonstone implements Runnable {
		...
		public void run() {
			...
			if (this.document.extractPatientNameAndDatesFromFirstLine(Readmission.DocumentDateStringFormats)) {
				System.out.print("Processing file (THREAD): " + tsetname + ":" + this.document.getName() + "...");
				long start = System.currentTimeMillis();
				...
				long end = System.currentTimeMillis();
				System.out.println("Done.  Elapsed: " + (end - start) + " milliseconds");
			}
		}
	}

	public void invokeMoonstoneTuffy(Document document) {
		Vector<Annotation> annotations = this.moonstone.getControl().applyNarrativeGrammarToText(document, true, true,
				true);
		annotations = this.moonstone.getSentenceGrammar().getDisplayedAnnotations();
		if (annotations != null) {
	...
	public void invokeMoonstone(Document document) {
		Vector<Annotation> targets = this.mexml.gatherTargetAnnotations(this.moonstone.getReadmission(), document,
				false);
		Vector<Annotation> expanded = Annotation.getNonNestedNonCoincidingAnnotations(targets, true);
		if (expanded != null) {
			for (Annotation target : expanded) {
				String concept = target.getConcept().toString();
				boolean isNegated = JavaFunctions.isNegated(target);
				ReadmissionSnippetResult tsr = new ReadmissionSnippetResult(this, document, target);
			}
		}
	...
	public void invokeMoonstoneARFF_BEFORE_5_15_2017(Document document) {
		...
		Vector<Annotation> annotations = msri.getControl().applyNarrativeGrammarToText(document, false, false, false);
		if (annotations != null) {
			for (String variable : this.moonstone.getReadmission().getAllEHostVariables()) {
	...
	// 5/15/2017 NEW: REMOVE DUPLICATE CONCEPT INSTANCES.
	public void invokeMoonstoneARFF(Document document) {
		MoonstoneRuleInterface msri = this.processor.moonstone;
		Vector<Annotation> annotations = msri.getControl().applyNarrativeGrammarToText(document, true, true, true);
		if (annotations != null) {
			annotations = this.processor.moonstone.getSentenceGrammar().getDisplayedAnnotations();
			for (String variable : this.moonstone.getReadmission().getAllEHostVariables()) {
				invokeMoonstoneARFF(document, variable, annotations);
			}
		}
	...
	public void invokeMoonstoneARFF(Document document, String variable, Vector<Annotation> annotations) {
		String pname = document.getPatientName();
		String answer = null;
		if (this.ehostPatientResults != null) {
			answer = this.ehostPatientResults.getPatientClassification(pname, variable);
		}
		if (this.processor.ARFFWithEHostAnswer && (answer == null || answer.contains("mention"))) {
			answer = "no mention";
			return;
		}
		String key = variable + ":" + pname;
		ARFFPatientVectorVariable apvv = this.processor.ARFFPatientVectorVariableHash.get(key);
		if (apvv == null) {
			apvv = new ARFFPatientVectorVariable(this, pname, variable, answer);
		}
		for (Annotation annotation : annotations) {
			Vector<String> allcontent = this.processor.featureSet.getRelevantConcepts(annotation);
			if (allcontent != null) {
				for (String content : allcontent) {
					apvv.addFeature(annotation, content);
				}
			}
		}
	}
	...
	public void storeTuffyEvidence() {
		if (this.processor.TuffyEvidenceFilename != null) {
			String fpath = this.moonstone.getResourceDirectoryName() + File.separatorChar
					+ this.processor.TuffyEvidenceFilename;
	...
	public void storeWEKAVariableFileStrings() {
		for (String variable : this.processor.moonstone.getReadmission().getSchema().getRelevantTypes()) {
			Vector<String> evalues = this.processor.moonstone.getReadmission().getTypeAttributeValues(variable);
			for (String evalue : evalues) {
				for (Enumeration<String> e = this.patientNameHash.keys(); e.hasMoreElements();) {
					String pname = e.nextElement();
					boolean confirmedInEHost = this.ehostPatientResults.patientHasClassification(pname, evalue);
					if (confirmedInEHost) {
						Vector<ReadmissionSnippetResult> sresults = this.patientSnippetResultHash.get(pname);
						if (sresults != null) {
	...
	public void storeSalomehPatientStatistics() {
		...
		for (String pname : pnames) {
			...
			for (String variable : rm.getAllEHostVariables()) {
				line += "\t";
				String codestr = null;
				for (String classification : rm.getTypeAttributeValues(variable)) {
					String key = pname + "@@" + classification;
					ReadmissionSummaryResult rsr = (ReadmissionSummaryResult) this.patientSummaryResultHash.get(key);
					if (rsr != null && rsr.confirmedBestCategory) {
						codestr = this.processor.SalomehClassificationCodeHash.get(classification);
						break;
					}
				}
				if (codestr == null) {
	...
	public void storeSalomehPatientStatisticsOLD() {
		...
		for (String pname : pnames) {
			Vector<ReadmissionSummaryResult> rsrs = (Vector<ReadmissionSummaryResult>) this.patientSummaryResultHash
					.get(pname);
			if (rsrs != null) {
				Collections.sort(rsrs, new ReadmissionSummaryResult.ReadmissionSummaryResultVariableValueSorter());
				for (ReadmissionSummaryResult rsr : rsrs) {
					int confirmed = rsr.confirmedBestCategory ? 1 : 0;
					String line = pname + "\t" + rsr.getSemanticVariable() + "\t" + rsr.getEHostValue() + "\t"
							+ confirmed + "\n";
					this.processor.SalomehPatientResultsSB.append(line);
				}
			}
	...
	public void printExcelStatisticsBestOnly(String header) {
		...
		for (String evalue : this.getAllTargetEHostValues()) {
			...
			line = casename + ed + evalue + ed + tp + ed + fp + ed + tn + ed + fn + ed + accuracy + ed + sensitivity
					+ ed + specificity + ed + fscore + "\n";
			sb.append(line);
		}
		String fname = this.processor.moonstone.getResourceDirectoryName() + File.separator
				+ "ReadmissionPatientStatistics.txt";
	...
	public void printExcelStatisticsAllMeasures(String header) {
		...
		for (String evalue : this.getAllTargetEHostValues()) {
			...
			for (String mlabel : ReadmissionCorpusProcessor.MatchMeasureLabels) {
				...
				fscore = getFmeasure(tp, tn, fp, fn);
				line += ed + tp + ed + fp + ed + tn + ed + fn + ed + accuracy + ed + sensitivity + ed + specificity + ed
						+ fscore;
			}
			sb.append(line + "\n");
		}
	...
	public void printVariableSummaryStatisticsBestOnly() {
		StringBuffer sb = new StringBuffer();
		String inputdirname = this.processor.moonstone.getStartupParameters().getPropertyValue("TextInputDirectory");
		String casename = null;
		String line = "Variable|TP|FP|TN|FN|Accuracy|Sensitivity|SpecificityF-Measure\n";
		sb.append(line);
		String ed = ReadmissionCorpusProcessor.ExcelDelimiter;
		for (String generalType : this.processor.moonstone.getReadmission().getSchema().getRelevantTypes()) {
			String mlabel = "best";
			String key = null;
			...
			line = generalType + ed + tp + ed + fp + ed + tn + ed + fn + ed + accuracy + ed + sensitivity + ed
					+ specificity + ed + fmeasure + "\n";
			sb.append(line);
		}
		String fname = this.processor.moonstone.getResourceDirectoryName() + File.separator
	...
	public void printLearningPatientTrainingAnswers() {
		...
		if (fname != null) {
			String fpath = this.processor.moonstone.getResourceDirectoryName() + File.separatorChar + fname;
	...
	public void addTuffyStringNEW(Annotation annotation) {
		...
		StringBuffer sb = new StringBuffer();
		Vector<String> features = FeatureSet.getRelevantPropositionalContent(annotation);
	...
	public String convertOldToNewEHostValue(String evalue) {
		String newname = this.processor.oldToNewGrammarEHostValueConversionHash.get(evalue);
		if (newname != null) {
			evalue = newname;
		}
		return evalue;
