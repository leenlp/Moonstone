...
public class Readmission {
	...
	public static String[] DocumentDateStringFormats = new String[] { "ddMMMyyyy", "MMddyyyy" };
	...
	private static Vector<String> InvalidHeaders = VUtils.arrayToVector(
			new String[] { "morse fall scale", "braden scale", "braden scale - for predicting pressure sore risk",
					"fall prevention interventions", "other risk factors", "for secondary diagnoses", "katz score" });

	private static String[] MoonstoneInvalidHeaderStrings = new String[] { "goal", "plan", "review" };
	...
	public void getNarrativeAnnotationMatchPercentage(MoonstoneRuleInterface msri) {
		...
		Vector<ReadmissionAnnotationInformationPacket> correctPackets = new Vector(0);
		Vector<ReadmissionAnnotationInformationPacket> incorrectPackets = new Vector(0);
		...
		// }
		if (msri.getWorkbench() != null && workbench.getAnalysis().getAllAnnotations() != null) {
			for (workbench.api.annotation.Annotation wba : workbench.getAnalysis().getAllAnnotations()) {

				KTAnnotation kta = wba.getKtAnnotation();
				KTClassMention ktcm = kta.getAnnotatedMention();
				Document doc = wba.getAnnotationCollection().getAnnotationEvent().getDocument();

				String dstr = doc.getText().substring(wba.getStart(), wba.getEnd() + 1);
				String astr = wba.getText();
				ReadmissionAnnotationInformationPacket packet = new ReadmissionAnnotationInformationPacket(this, doc,
						kta);
				...
				sentenceHash.put(packet.MoonstoneSnippet, packet.MoonstoneSnippet);

				if (packet.MoonstoneWasCorrect) {
					correctPackets.add(packet);
					HUtils.incrementCount(hitCountHash, packet.EHostConcept);
					VUtils.pushIfNotHashVector(matchValues, packet.EHostConcept, packet.MoonstoneSnippet);
					// 11/14/2015
					this.countCorrectIncorrectRules(packet.MoonstoneDocumentAnnotations, true);
				} else {
					...
					VUtils.pushIfNotHashVector(missValues, packet.EHostConcept, packet.MoonstoneSnippet);
					missDocs.put(packet.MoonstoneSnippet, doc.getName());
					this.countCorrectIncorrectRules(packet.MoonstoneDocumentAnnotations, false);
		...
		try {
			...
			for (String concept : concepts) {
				float annotationCount = HUtils.getCount(conceptCountHash, concept);
				...
				text += "\n\n\tHits (" + hitCount + "):\n";
				Vector<String> values = (Vector<String>) matchValues.get(concept);
				if (values != null) {
	...
	Vector<Annotation> getDocumentAnnotations(String text) {
		...
		Vector<Annotation> annotations = msri.getControl().applyNarrativeGrammarToText(doc, true, true, false);
		return this.msri.getControl().getDocumentGrammar().getDisplayedAnnotations();
	}
	...
	public void initialize() {
		try {
			...
			this.schema = new AMIR01Schema();
			String sname = this.msri.getStartupParameters().getPropertyValue(ProjectSchemaVersionParameter);
			if ("668".equals(sname)) {
			...
			} else if ("owl-pneumonia".equals(sname)) {
				this.schema = new OWLPneumoniaSchema();
			}
			...
			if (schema.conceptConversionMap != null) {
				for (int i = 0; i < schema.conceptConversionMap.length; i++) {
					...
					String ec = entry[0];
					String mc = entry[1];
					VUtils.pushIfNotHashVector(this.MoonstoneToEHostConversionHash, mc, ec);
					VUtils.pushIfNotHashVector(this.EHostToMoonstoneConversionHash, ec, mc);
					String[] strs = ec.split(":");
					if (strs.length == 2) {
						String vvalue = strs[0];
						String evalue = strs[1];
						VUtils.pushIfNotHashVector(this.EHostToMoonstoneConversionHash, evalue, mc);
						this.EHostConceptVariableConversionHash.put(evalue, vvalue);
					}

					this.allMoonstoneTargetConcepts = VUtils.addIfNot(this.allMoonstoneTargetConcepts, mc);
					this.allEHostTargetConcepts = VUtils.addIfNot(this.allEHostTargetConcepts, ec);
				}
			}
			...
			if (schema.negatedConceptMap != null) {
				for (int i = 0; i < schema.negatedConceptMap.length; i++) {
					...
					VUtils.pushIfNotHashVector(negatedConceptLists, entry[0], entry[1]);
					VUtils.pushIfNotHashVector(negatedConceptLists, entry[1], entry[0]);
			...
			if (schema.relevantAttributes != null) {
				for (int i = 0; i < schema.relevantAttributes.length; i++) {
					...
					relevantAttributeHash.put(entry.toLowerCase(), entry);
				}
			}
			...
			if (schema.typeAttributeValueMap != null) {
				for (int i = 0; i < schema.typeAttributeValueMap.length; i++) {
					...
					for (int j = 1; j < entry.length; j++) {
						...
						VUtils.pushHashVector(this.typeAttributeValueHash, type, value);
	...
	public static boolean workbenchAnnotationHasAttribute(workbench.api.annotation.Annotation wba, String aname,
			String value) {
		String v = (String) wba.getAttributeValue(aname);
		return value.equals(v);
	}

	public String extractSchemaValueFromMoonstoneConcept(Object concept, boolean istype) {
		String value = null;
		if (concept != null) {
	...
	public boolean annotationIsRelevant(Annotation annotation, boolean allowNegation) {
		return (annotation != null && (allowNegation || !this.annotationIsNegated(annotation))
				&& conceptIsRelevant(annotation.getConcept()) && annotation.getText().length() < 200);
	}

	public Annotation getLowestRelevantAnnotation(Annotation annotation, boolean allowNegation) {
		Annotation lowest = null;
		if (annotationIsRelevant(annotation, allowNegation)) {
			Annotation child = getLowestRelevantAnnotation(annotation.getSingleChild(), allowNegation);
			lowest = (child != null ? child : annotation);
		}
		return lowest;
	}

	public Vector<Annotation> gatherAllRelevantAnnotations(Annotation annotation, boolean lowest,
			boolean allowNegation) {
		return gatherAllRelevantAnnotations(annotation, new Hashtable(), lowest, allowNegation);
	}
	...
	// LivesAtHome.
	private Vector<Annotation> gatherAllRelevantAnnotations(Annotation annotation, Hashtable<String, String> chash,
			boolean lowest, boolean allowNegation) {
		Vector<Annotation> annotations = null;
		if (annotation != null && annotationIsRelevant(annotation, allowNegation)) {
		...
		if (annotation.hasChildren()) {
			for (Annotation child : annotation.getChildAnnotations()) {
				annotations = VUtils.append(annotations,
						gatherAllRelevantAnnotations(child, chash, lowest, allowNegation));
			}
		}
		return annotations;
	}
	...
	public void countCorrectIncorrectRules(Vector<Annotation> annotations, boolean correct) {
		if (annotations != null) {
	...
	public void countCorrectIncorrectRules(Annotation annotation, boolean correct) {
		Rule rule = annotation.getRule();
		if (rule != null && !rule.isTerminal()) {
			Hashtable hash = (correct ? this.correctRuleCountHash : this.incorrectRuleCountHash);
	...
	public boolean annotationIsNegated(Annotation annotation) {
		return (annotation != null && JavaFunctions.hasPropertyValue(annotation, "directionality", "negated"));
	}
