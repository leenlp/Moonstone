...
public class WEWManager {
	...
	public static ResultTable load(final DocumentReference schema, final List<DocumentReference> documents,
			final List<AnnotatorReference> primaryAnnotators, final List<AnnotatorReference> secondaryAnnotators,
		...
		for (int i = 0; i < documents.size(); i++) {
			...
			annotationFileNameIndexHash.put(secondary.getName(), new Integer(i));
			String dtext = WEWManager.readDocumentReference(dr, false);
			documentTextHash.put(dr, dtext);
			WEWManager.readAnnotationCollection(analysis, dr, primary, params, kio, null);
			WEWManager.readAnnotationCollection(analysis, dr, secondary, params, kio, null);
		}
		...
		if (analysis.getAllAnnotationEvents() != null) {
			for (AnnotationEvent ae : analysis.getAllAnnotationEvents()) {
				...
				if (ac.getAnnotations() != null) {
					for (workbench.api.annotation.Annotation wba : ac.getAnnotations()) {
						if ("DocumentClass".equals(wba.getType().getName())) {
							continue;
						}
						AnnotationImpl ai = new AnnotationImpl();
						String cstr = wba.getClassificationValue().toString();
						ConceptImpl cimpl = getConceptImpl(cstr, rmap);
						String dname = ac.getAnnotationEvent().getDocumentName();
						Integer index = annotationFileNameIndexHash.get(dname);
						int i = index.intValue();
						DocumentReference dr = documents.get(i);
						AnnotatorReference primary = primaryAnnotators.get(i);
						String atext = WEWManager.getAnnotationText(documentTextHash.get(dr), wba);
						ai.setText(atext);
		...
		Map<ConceptReference, Map<OutcomeType, List<Annotation>>> resultsSet = rt.getResults();
		Map<ConceptReference, Map<CalculationType, Double>> statisticalSet = rt.getStatisticsalSet();
		Map<CalculationType, Double> statisticalSummary = rt.getStatisticalSummary();
		...
		if (cm != null) {
			...
			for (int i = 0; i < numberOfValues; i++) {
				...
				if (cimpl == null) {
					System.out.println("UNABLE TO FIND CONCEPTIMPL FOR \"" + concept + "\"");
				}
				...
				for (OutcomeType ot : OutcomeType.values()) {
					...
					wl = mvs.getAnnotationHashResults(or, workbench.api.AnnotatorType.primary);
					if (wl != null) {
					...
					wl = mvs.getAnnotationHashResults(or, workbench.api.AnnotatorType.secondary);
					if (wl != null) {
						for (workbench.api.annotation.Annotation a : wl) {
							AnnotationImpl ai = (AnnotationImpl) rmap.get(a);
							if (ai != null) {
	...
	public static void readKnowtatorSchemaFile(final DocumentReference dr, KnowtatorIO kio) throws Exception {
		if (dr.getName() != null) {
	...
	public static void readAnnotationCollection(final Analysis analysis, final DocumentReference dr,
			final AnnotatorReference ai, final Params params, KnowtatorIO kio, String doctext) throws Exception {
		String format = (String) params.getParams().get("format");
		int x = 1;
		String afilestr = WEWManager.readDocumentReference(ai, true);
		boolean isPrimary = WEWManager.annotatorIsPrimary(ai.getAnnotatorType());
		
		String docname = dr.getName();
		
		if (docname.toLowerCase().contains("afmlfr4o0")) {
			x = 1;
		}
		
		analysis.addAnnotationCollection(dr.getName(), ai.getName(), afilestr, format, isPrimary, kio, doctext);
	}

	private static String getAnnotationText(final String dtext, final workbench.api.annotation.Annotation wba) {
		String atext = "*";
		if (dtext != null) {
	...
	public static String readDocumentReference(final DocumentReference dr, final boolean doTrim) throws Exception {
		String text = "";
		try {
			...
			text = String.valueOf(cbuf, 0, size + 1);
			if (doTrim) {
				text = text.trim();
			}
	...
	protected static boolean formatTypeisGRAF(final Params params) {
		if (params != null) {
			...
			return (str == null || str.toLowerCase().equals(WEWManager.FormatTypeGRAF.toString().toLowerCase()));
		}
		return false;
	}
	...
	protected static boolean formatTypeisKnowtator(final Params params) {
		if (params != null) {
			String str = (String) params.getParams().get(WEWManager.FormatType);
			return (str == null || str.toLowerCase().equals(WEWManager.FormatTypeKnowtator.toString().toLowerCase()));
		}
		return false;
	}
	...
	protected static void storeParamsAsKnowledgeEngineProperties(Params params, KnowledgeEngine ke) {
		for (String property : params.getParams().keySet()) {
			Object value = params.getParams().get(property);
			ke.getStartupParameters().setPropertyValue(property, value);
		}
	}
