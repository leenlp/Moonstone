...
public class ConstraintMatch {
	...
	public void applyConstraintToMatchedPairs() {
		...
		if (avs != null && this.analysis.getAllPrimarySnippetAnnotations() != null
			...
			Annotation.resetContainsClassificationMatch(this.analysis.getAllPrimarySnippetAnnotations());
			Annotation.resetContainsClassificationMatch(this.analysis.getAllSecondarySnippetAnnotations());

			this.matchedValueStatistics = new MatchedValueStatistics[this.alternativeValues.size()];
			...
			Vector<Variable> vars = VUtils.listify(avar, vvar, avar1, avar2, cvar, loopvar);
			for (int i = 0; i < this.alternativeValues.size(); i++) {
				String value = this.alternativeValues.elementAt(i);
				...
				MatchedValueStatistics mvs = new MatchedValueStatistics(this, value);
				this.matchedValueStatistics[i] = mvs;
				vvar.bind(value);
				for (Annotation primary : this.analysis.getAllPrimarySnippetAnnotations()) {
					...
					boolean hasValidationIncorrect = JavaFunctions.annotationHasAttributeValue(primary, "validation",
							"incorrect");
					...
					String pname = pclass.toString().toLowerCase();
					String dname = primary.getDocumentName().toLowerCase();
					cvar.bind(pclass);
					loopvar.bind("TP-FN-PRIMARY");
					boolean foundPrimaryMatch = this.constraintPacket.getConstraint().doTestConstraint(vars);
					...
					if (foundPrimaryMatch) {
						...
						if (primary.getOverlappingAnnotationPairs() != null) {
							for (OverlappingAnnotationPair pair : primary.getOverlappingAnnotationPairs()) {
								Annotation secondary = pair.getSecondaryAnnotation();
								...
								if (this.constraintPacket.getConstraint().doTestConstraint(vars)) {
									...
									secondary.setContainsClassificationMatch(true);
									mvs.incrementTruePositive(primary, secondary);
									this.cumulativeTruePositiveCount++;
									String key = getDocumentMatchedPairKey(docname, value, OutcomeResult.TP);
									VUtils.pushHashVector(this.primaryMatchedDocumentAnnotationHash, key, primary);
									VUtils.pushHashVector(this.secondaryMatchedDocumentAnnotationHash, key, secondary);
						...
						if (!foundSecondaryMatch) {
							...
							if (this.analysis.isAnnotationFPtoTPMatch(primary)) {
								mvs.incrementTrueNegative(primary, null);
								...
								String key = getDocumentMatchedPairKey(docname, value, OutcomeResult.FN);
								VUtils.pushHashVector(this.primaryMatchedDocumentAnnotationHash, key, primary);
								VUtils.pushHashVector(this.falseAnnotationHash, "FN:" + value, primary);
							}
						}
					}
				}

				for (Annotation secondary : this.analysis.getAllSecondarySnippetAnnotations()) {
					...
					if (secondary.isVisited() || secondary.overlappingSetContainsMatchedClassification()) {
						continue;
					}
					...
					cvar.bind(sclass);
					loopvar.bind("FP");
					boolean foundSecondaryMatch = this.constraintPacket.getConstraint().doTestConstraint(vars);
					if (foundSecondaryMatch) {
						...
						if (this.analysis.isAnnotationFPtoTPMatch(secondary)) {
							...
							String key = getDocumentMatchedPairKey(docname, value, OutcomeResult.FP);
							VUtils.pushHashVector(this.secondaryMatchedDocumentAnnotationHash, key, secondary);

							// 1/14/2015
							VUtils.pushHashVector(this.falseAnnotationHash, "FP:" + value, secondary);
						}
					}
				}
			}
		}
		Annotation.setVisited(this.analysis.getAllPrimarySnippetAnnotations(), false);
		Annotation.setVisited(this.analysis.getAllSecondarySnippetAnnotations(), false);
	}
	...
	public static String getDocumentMatchedPairKey(String docname, String value, OutcomeResult result) {
	...
	public Vector getHashKey() {
		Vector key = VUtils.listify(this.getWorkbenchAPIObject(), this.constraintPacket.getConstraint());
		return key;
	}
	...
	public String getFirstColumnNameAll(int row) {
		Object value = (this.alternativeValues != null ? this.alternativeValues.elementAt(row) : null);
		String str = "*";
	...
	public Vector<Annotation> getAllMatchedDocumentAnnotations(String value, OutcomeResult result,
			AnnotatorType atype) {
		Vector<Annotation> allannotations = null;
		if (this.analysis.getAllDocuments() != null) {
			for (Document doc : this.analysis.getAllDocuments()) {
				Vector<Annotation> annotations = this.getMatchedDocumentAnnotations(doc.getName(), value, result,
						atype);
				// allannotations = VUtils.append(allannotations, annotations);

				// 8/28/2017: SHOULDN'T NEED THIS...
				allannotations = VUtils.appendIfNot(allannotations, annotations);
			}
		}
		return allannotations;
	}

	public Vector<Annotation> getMatchedDocumentAnnotations(String docname, String value, OutcomeResult result,
			AnnotatorType atype) {
		String key = ConstraintMatch.getDocumentMatchedPairKey(docname, value, result);
	...
	public String toString() {
		String str = "<CM: ConstraintString=\"" + this.getConstraintPacket().getConstraint().getConstraintString()
				+ "\">";
		return str;
	}
	...
	public static ConstraintMatch getConstraintMatchNamed(Vector<ConstraintMatch> cms, String name) {
	...
	/// 3/30/2018:  IMPORTED FROM VINCI FOR MAKING MISMATCH SPREADSHEETS
	// 2/2/2018: For creation mention spreadsheet
	private static String SpreadsheetFilenameHidden = "SpreadsheetFilenameHidden";
	private static String SpreadsheetFilenameVisible = "SpreadsheetFilenameVisible";

		public void writeSpreadsheetFileVisible() {
			StringBuffer sb = new StringBuffer();
			sb.append("CLASSIFICATION|MATCH|MENTION|CONTEXT|DOCUMENT\n");
			Vector<ConstraintMatchPrintable> cps = new Vector(0);
			addToSpreadsheet(cps, this.primaryMatchedDocumentAnnotationHash, 20);
			addToSpreadsheet(cps, this.secondaryMatchedDocumentAnnotationHash, 20);

			Collections.sort(cps, new ConstraintMatchPrintableMatchTypeSorter());
			Collections.sort(cps, new ConstraintMatchPrintableTargetSorter());

			// Collections.sort(cps, new ConstraintMatchPrintableMentionSorter());
			for (ConstraintMatchPrintable cp : cps) {
				String str = cp.target + "|" + cp.matchType + "|" + cp.mention
						+ "|" + cp.sentence + "|" + cp.documentName + "|\n";
				sb.append(str);
			}
			String resourcedir = KnowledgeEngine.getCurrentKnowledgeEngine().getStartupParameters().getResourceDirectory();
			String fname = resourcedir + File.separatorChar + SpreadsheetFilenameVisible;
			FUtils.writeFile(fname, sb.toString());
		}

		// 2/16/2018: Long mention spreadsheet, match hidden, larger context, as
		// requested by Salomeh
		public void writeSpreadsheetFileHidden() {
			StringBuffer sbNoMatch = new StringBuffer();
			sbNoMatch.append("CLASSIFICATION|CORRECT? (1 or 0)|MENTION|CONTEXT|DOCUMENT\n");
			StringBuffer sbMatch = new StringBuffer();
			sbMatch.append("CLASSIFICATION|MATCH|MENTION|CONTEXT|DOCUMENT|\n");
			Vector<ConstraintMatchPrintable> cps = new Vector(0);

			addToSpreadsheet(cps, this.primaryMatchedDocumentAnnotationHash, 50);
			addToSpreadsheet(cps, this.secondaryMatchedDocumentAnnotationHash, 50);
			...
			for (ConstraintMatchPrintable cp : cps) {
				String str = cp.target + "|    |" + cp.mention + "|" + cp.sentence + "|" +
						cp.documentName + "|\n";
				sbNoMatch.append(str);
			}
			String resourcedir = KnowledgeEngine.getCurrentKnowledgeEngine().getStartupParameters().getResourceDirectory();
			String fname = resourcedir + File.separatorChar + SpreadsheetFilenameHidden;
			FUtils.writeFile(fname, sbNoMatch.toString());
			
			for (ConstraintMatchPrintable cp : cps) {
				String str = cp.target + "|" + cp.matchType + "|" + cp.mention + "|" + cp.sentence + "|" + cp.documentName + "|\n";
				sbMatch.append(str);
			}
			fname = resourcedir + File.separatorChar + SpreadsheetFilenameVisible;
		...
		private class ConstraintMatchPrintable {
			...
			ConstraintMatchPrintable(Annotation a, String mtype, int sbuflen) {
				...
				this.documentName = a.getDocumentName();
