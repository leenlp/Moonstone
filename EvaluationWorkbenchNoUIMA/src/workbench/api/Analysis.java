...
public class Analysis {
	...
	private Hashtable<String, String> EHostConceptConversionHash = new Hashtable();
	...
	private static String EHostConceptConversionsName = "EHostConceptConversions";
	...
	public Analysis(WBGUI wbgui, EvaluationWorkbench arrtool) throws Exception {
		...
		this.primaryAnnotator = new Annotator(arrtool.getAnalysis().getPrimaryAnnotator().getName(), true);
		this.secondaryAnnotator = new Annotator(arrtool.getAnalysis().getSecondaryAnnotator().getName(), false);
	...
	public Analysis(TypeSystem ts) throws Exception {
		CurrentAnalysis = this;
		this.knownledgeEngine = KnowledgeEngine.getCurrentKnowledgeEngine(true);
		this.getEHostConceptConversions();
		if (ts == null) {
		...
		if (this.primaryAnnotator == null) {
			String firstAnnotator = this.knownledgeEngine.getStartupParameters().getPropertyValue("firstAnnotator");
			String secondAnnotator = this.knownledgeEngine.getStartupParameters().getPropertyValue("secondAnnotator");
		...
		String primaryAnnotationDirectory = this.knownledgeEngine.getStartupParameters()
				.getPropertyValue(StartupParameters.AnnotationInputDirectoryFirstAnnotator);
		String secondaryAnnotationDirectory = this.knownledgeEngine.getStartupParameters()
				.getPropertyValue(StartupParameters.AnnotationInputDirectorySecondAnnotator);
		if (primaryAnnotationDirectory != null && secondaryAnnotationDirectory != null
				&& primaryAnnotationDirectory.equals(secondaryAnnotationDirectory)) {
	...
	public void readAnnotationsFromOldWorkbench(EvaluationWorkbench arrtool) {
		for (workbench.arr.AnnotationEvent evae : arrtool.getAnalysis().getAnnotationEvents()) {
			...
			// ae.getDocument());
			this.putNamedDocumentHash(ae.getDocument().getName(), ae.getDocument());
			this.documentAnnotationEventHash.put(ae.getDocument().getName(), ae);

			AnnotationCollection primary = ae.getPrimaryAnnotationCollection();
			AnnotationCollection secondary = ae.getSecondaryAnnotationCollection();
			this.allAnnotations = VUtils.append(this.allAnnotations, primary.getAnnotations());
			this.allAnnotations = VUtils.append(this.allAnnotations, secondary.getAnnotations());
			this.allAnnotationCollections = VUtils.add(this.allAnnotationCollections, primary);
			this.allAnnotationCollections = VUtils.add(this.allAnnotationCollections, secondary);
		}
		resolveOldWorkbenchRelations();
	}

	public void resolveOldWorkbenchRelations() {
		for (Enumeration<EVAnnotation> e = this.oldToNewAnnotationHash.keys(); e.hasMoreElements();) {
			...
			if (eva.getRelationObjects() != null) {
				...
				for (RelationObject ro : eva.getRelationObjects()) {
					Annotation relatum = this.oldToNewAnnotationHash.get(ro.getRelatum());
					annotation.addRelation(ro.getRelation(), annotation, relatum);
				}
			}
		}
	}
	...
	public void addAnnotationCollection(String docname, String afilename, String fstr, String format, boolean isPrimary,
			KnowtatorIO kio, String doctext) throws Exception {
		...
		if (format.equals(FormatType.GRAF.toString().toLowerCase())) {
			...
			VUtils.pushHashVector(this.documentAnnotationCollectionMap, docname, ac);
		} else if (format.equals(FormatType.Knowtator.toString().toLowerCase())) {
			// 3/9/2015
			if (this.isSingleAnnotationSet()) {
				Annotator annotator = (isPrimary ? this.getPrimaryAnnotator() : this.getSecondaryAnnotator());
	...
	// 7/11/2017: For use with MySQL
	public void postProcessAnnotationCollectionsMySQL() throws Exception {
		if (this.getAllAnnotationEvents() != null) {
			for (AnnotationEvent ae : this.getAllAnnotationEvents()) {
				String dname = ae.getDocumentName();
				Document doc = new Document(dname, (String) null);
				ae.setDocument(doc);
				ae.setDocumentID(ae.getDocumentID());
				...
				this.allAnnotations = VUtils.append(this.allAnnotations,
						ae.getPrimaryAnnotationCollection().getAnnotations());
				this.allAnnotations = VUtils.append(this.allAnnotations,
						ae.getSecondaryAnnotationCollection().getAnnotations());
			}
			this.allAnnotationEvents = HUtils.getElements(this.documentAnnotationEventHash);
			if (this.allAnnotationEvents != null) {
				Collections.sort(this.allAnnotationEvents, new AnnotationEvent.DocumentNameSorter());
				this.gatherOverlappingAnnotations();
				this.gatherAllClassifications();
			} else {
	...
	public void postProcessAnnotationCollections() throws Exception {
		for (Enumeration<String> e = this.documentAnnotationCollectionMap.keys(); e.hasMoreElements();) {
			String dname = e.nextElement();
			if (dname.contains("2484")) {
				int x = 1;
			}
			Vector<AnnotationCollection> v = this.documentAnnotationCollectionMap.get(dname);
			if (v != null) {
				AnnotationEvent ae = new AnnotationEvent(this, (Document) null, dname);

				// 3/20/2017: PROBLEM: I'm now working with thousands of
				// documents...
	...
	public void updateStatistics() {
		...
		if (wao != null && cp != null) {
			...
			if (this.getWorkbenchGUI() != null) {
				this.getWorkbenchGUI().getStatisticsPanel().getModel().fireTableDataChanged();
			}
		}
	}
	...
	public void gatherAllClassifications() {
		...
		String cstr = this.getKnownledgeEngine().getStartupParameters().getPropertyValue("TargetWorkbenchConcepts");
		...
		if (cstr != null) {
			...
			for (Annotation annotation : this.allSnippetAnnotations) {
				...
				if (cv == null || cv.contains(value.toString())) {
					if (value == null) {
						int x = 1;
					}
					chash.put(value.toString(), value);
				}
			}
	...
	public void gatherOverlappingAnnotations() {
		...
		if (this.allAnnotationEvents != null) {
			...
			for (AnnotationEvent ae : this.allAnnotationEvents) {
				...
				this.allPrimarySnippetAnnotations = VUtils.append(this.allPrimarySnippetAnnotations,
						pac.getSnippetAnnotations());
				this.allSecondarySnippetAnnotations = VUtils.append(this.allSecondarySnippetAnnotations,
						sac.getSnippetAnnotations());
			}
			if (this.allPrimarySnippetAnnotations != null) {
				// 2/15/2018
				for (Annotation annotation : this.allPrimarySnippetAnnotations) {
					annotation.setValidatedPrimaryAnnotation();
				}
				Collections.sort(this.allPrimarySnippetAnnotations, new Annotation.DocumentPositionSorter());
			}
			if (this.allSecondarySnippetAnnotations != null) {
				Collections.sort(this.allSecondarySnippetAnnotations, new Annotation.DocumentPositionSorter());
			}
			this.allSnippetAnnotations = VUtils.appendNew(this.allPrimarySnippetAnnotations,
					this.allSecondarySnippetAnnotations);
			if (this.allSnippetAnnotations != null) {
				Collections.sort(this.allSnippetAnnotations, new Annotation.DocumentPositionSorter());
			}
			for (AnnotationEvent ae : this.allAnnotationEvents) {
	...
	public void gatherOverlappingAnnotations(AnnotationEvent ae) {
		...
		AnnotationCollection pac = ae.getPrimaryAnnotationCollection();
		AnnotationCollection sac = ae.getSecondaryAnnotationCollection();
		if (pac.getSnippetAnnotations() == null || sac.getSnippetAnnotations() == null) {
			return;
		}
		...
		for (Annotation a1 : pac.getSnippetAnnotations()) {
			for (Annotation a2 : sac.getSnippetAnnotations()) {
				...
				boolean hasStrictOverlap = AnnotationCollection.hasStrictOverlap(a1, a2);
				if (overlap > 0) {
					OverlappingAnnotationPair pair = new OverlappingAnnotationPair(a1, a2, hasStrictOverlap);
					if (a1.getClassificationValue() != null) {
						VUtils.pushHashVector(matchedClassificationOverlappingPairHash, a1.getClassificationValue(),
								pair);
					} else {
						x = 1;
					}
					this.allOverlappingAnnotationPairs = VUtils.add(this.allOverlappingAnnotationPairs, pair);
				}
			}
		}
	}
	...
	public static boolean hasStrictOverlap(Annotation a1, Annotation a2) {
		if (a1.getSpans() == null || a2.getSpans() == null || a1.getSpans().size() != a2.getSpans().size()) {
			return false;
		}
	...
	public void addAnnotationEvent(AnnotationEvent ae) {
		this.allAnnotationEvents = VUtils.add(this.allAnnotationEvents, ae);
	}
	...
	public void setSelectedAnnotation(Annotation selectedAnnotation) throws Exception {
	...
	public void setSelectedConstraintMatch(ConstraintMatch selectedConstraintMatch) {
	...
	public AnnotationEvent getSelectedAnnotationEvent() {
		if (this.selectedDocument != null) {
			return this.documentAnnotationEventHash.get(this.selectedDocument.getName());
		}
		return null;
	}
	...
	public AnnotationCollection getNonselectedAnnotationCollection() {
		AnnotationEvent ae = this.getSelectedAnnotationEvent();
		if (this.selectedAnnotator != null && ae != null) {
			AnnotationCollection ac = ae.getAnnotationCollection(this.selectedAnnotator);
			if (ac.isPrimary()) {
	...
	public void appendKnowtatorAnnotations(Vector<Annotation> annotations) {
		this.knowtatorAnnotations = VUtils.append(this.knowtatorAnnotations, annotations);
	}

	public void attachDocumentsToKnowtatorAnnotationCollections(Annotator annotator) throws Exception {
		if (this.knowtatorIO.getAnnotationCollections() != null) {
			for (AnnotationCollection ac : this.knowtatorIO.getAnnotationCollections()) {
	...
	public void pushDocumentAnnotationCollectionMap(Object key, AnnotationCollection ac) {
		String astr = ac.getAnnotator().toString();
		Vector v = this.documentAnnotationCollectionMap.get(key);
		if (v != null && v.size() > 1) {
			int x = 1;
		} else {
			int x = 1;
		}
		VUtils.pushIfNotHashVector(this.documentAnnotationCollectionMap, key, ac);
	}
	...
	public Vector<OverlappingAnnotationPair> getOverlappingAnnotationPairsByClassification(String classification) {
		return this.matchedClassificationOverlappingPairHash.get(classification);
	}
	...
	public WorkbenchAPIObject getUserSelectedWorkbenchAPIObject() {
		if (this.userSelectedWorkbenchAPIObject == null) {
			if (this.getTypeSystem() != null && this.getTypeSystem().getRootType() != null) {
				this.setUserSelectedWorkbenchAPIObject(this.getTypeSystem().getRootType());
			}
		}
	...
	public void setDefaultConstraintPacket(WorkbenchAPIObject wao) {
		...
		if (wao instanceof Type) {
			dcp = this.getConstraintPacket(ConstraintPacket.DefaultClassificationConstraintName);
		} else if (wao instanceof Attribute) {
			dcp = this.getConstraintPacket(ConstraintPacket.DefaultAttributeValueConstraintName);
		}
	...
	public ConstraintMatch getConstraintMatch(String cmname) {
		for (Enumeration<ConstraintMatch> e = this.matchedConstraintHash.elements(); e.hasMoreElements();) {
	...
	public String getNormalizedAnnotatorName(String aname) {
		if (this.knowtatorIO.getAnalysis().readAnnotationCollectionFileIsPrimary()
				&& this.getPrimaryAnnotator().containsName(aname)) {
			return "primary";
		} else if (!this.knowtatorIO.getAnalysis().readAnnotationCollectionFileIsPrimary()
	...
	public boolean isAnnotationFPtoTPMatch(Annotation annotation) {
		...
		if (annotation != null && annotation.getClassificationValue() != null) {
			...
			String cstr = annotation.getClassificationValue().toString().toLowerCase();
			String value = key + ":" + cstr + "==" + text;
			boolean rv = this.FPToTPTextHash.get(value) != null;
			if (rv) {
				int x = 1;
			}
			return rv;
	...
	public void recordAnnotationFPtoTPText() {
		...
		if (a != null && a.getClassificationValue() != null) {
			...
			String cstr = a.getClassificationValue().toString().toLowerCase();
			String value = key + ":" + cstr + "==" + text;
	...
	public void readFPToTPAnnotationTextsFromFile() {
		String rdir = this.knownledgeEngine.getStartupParameters().getResourceDirectory();
	...
	public void storeAnnotationFPtoTPTextsToFile() {
		if (this.FPToTPTextHash != null && !this.FPToTPTextHash.isEmpty()) {
			...
			for (String text : (Vector<String>) HUtils.getKeys(this.FPToTPTextHash)) {
	...
	public void putDocumentAnnotationEventHash(String dname, AnnotationEvent ae) {
		this.documentAnnotationEventHash.put(dname, ae);
	}

	public int getPrimaryRunID() {
		return primaryRunID;
	}

	public void setPrimaryRunID(int primaryRunID) {
		this.primaryRunID = primaryRunID;
	}

	public int getSecondaryRunID() {
		return secondaryRunID;
	}

	public void setSecondaryRunID(int secondaryRunID) {
		this.secondaryRunID = secondaryRunID;
	}
	
	public String getEHostConceptConversion(String cstr) {
		String newstr = this.EHostConceptConversionHash.get(cstr);
		if (newstr != null ) {
			cstr = newstr;
		}
		return cstr;
	...
	public void getEHostConceptConversions() {
		String cvstr = KnowledgeEngine.currentKnowledgeEngine.getStartupParameters()
				.getPropertyValue(EHostConceptConversionsName);
		if (cvstr != null) {
			String[] cstrs = cvstr.split(";");
			if (cstrs != null) {
				for (int i = 0; i < cstrs.length; i++) {
					String cstr = cstrs[i];
					String[] cpair = cstr.split(",");
					if (cpair.length == 2) {
						this.EHostConceptConversionHash.put(cpair[0], cpair[1]);
					}
				}
			}
		}
