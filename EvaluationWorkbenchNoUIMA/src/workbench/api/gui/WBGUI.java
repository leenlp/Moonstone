...
public class WBGUI extends JPanel implements ActionListener {
	...
	protected static Object[][] menuItemInfo = { { "operations", "printStatistics", "Print Statistics" },
			{ "operations", "writeStatisticsToFile", "Write Statistics to File" },
			{ "operations", "writeMentionsToFileVisible", "Write Mentions To File (Visible)" },
			{ "operations", "writeMentionsToFileHidden", "Write Mentions To File (Hidden)" } };

	public WBGUI(Analysis analysis, EvaluationWorkbench wb, boolean visible) throws Exception {
		WorkbenchGUI = this;
		if (analysis == null) {
		...
		int x = 1;
		if (KnowledgeEngine.getCurrentKnowledgeEngine().getStartupParameters().isPropertyTrue("CreateWorkbenchGUI")) {
	...
	public void initializeGraphics(boolean visible) throws Exception {
		...
		JScrollPane statjsp = new JScrollPane(this.statisticsPanel, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,
		...
		JScrollPane annjsp = new JScrollPane(this.annotationPanel, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,
				ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		...
		JScrollPane attrjsp = new JScrollPane(this.attributePanel, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,
				ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		jsp.setMinimumSize(d);
		jsp.setPreferredSize(d);
		...
		JSplitPane arp = new JSplitPane(JSplitPane.VERTICAL_SPLIT, annjsp, attrtpane);

		this.annotationTypeTabbedPane.addTab("Annotations", arp);

		d = new Dimension(400, 400);
		this.constraintPanel = new ConstraintPanel(this.analysis);
		JScrollPane conjsp = new JScrollPane(this.constraintPanel, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,
				ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		d = new Dimension(400, 400);
		...
		JScrollPane typejsp = new JScrollPane(this.typePanel, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,
				ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		...
		JScrollPane reljsp = new JScrollPane(this.relationPanel, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,
				ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		...
		d = new Dimension(900, 400);
		JSplitPane statsp = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, statjsp, this.annotationTypeTabbedPane);
		statsp.setMinimumSize(d);
		statsp.setPreferredSize(d);

		this.primaryDocumentPanel = new DocumentPanel(analysis, analysis.getPrimaryAnnotator());
		this.secondaryDocumentPanel = new DocumentPanel(analysis, analysis.getSecondaryAnnotator());
		JSplitPane dsp = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, this.primaryDocumentPanel,
				this.secondaryDocumentPanel);
		...
		JScrollPane docjsp = new JScrollPane(dsp, ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED,
				ScrollPaneConstants.HORIZONTAL_SCROLLBAR_AS_NEEDED);
		d = new Dimension(1000, 500);
		docjsp.setMinimumSize(d);
		docjsp.setPreferredSize(d);

		JSplitPane wbsp = new JSplitPane(JSplitPane.VERTICAL_SPLIT, statsp, docjsp);
	...
	private void setTitle() {
		...
		if (this.analysis.getSelectedConstraintPacket() != null) {
			str += "MatchCriterion=" + this.analysis.getSelectedConstraintPacket().getName();
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		} else if ("writeMentionsToFileVisible".equals(cmd)) {
			this.getAnalysis().getSelectedConstraintMatch().writeSpreadsheetFileVisible();
		} else if ("writeMentionsToFileHidden".equals(cmd)) {
			this.getAnalysis().getSelectedConstraintMatch().writeSpreadsheetFileHidden();
		}
	}
	...
	public void fireAllDataUpdates() {
		if (!firingAllDataUpdates) {
			try {
				...
				this.primaryDocumentPanel.setCaretPosition(this.analysis.getSelectedAnnotation());
				this.secondaryDocumentPanel.setCaretPosition(this.analysis.getSelectedAnnotation());
	...
	public void setAnalysis(Analysis analysis) {
		try {
			this.analysis = analysis;
			this.primaryDocumentPanel.setAnalysis(analysis);
			this.secondaryDocumentPanel.setAnalysis(analysis);
			this.constraintPanel.setAnalysis(analysis);
			this.constraintPanel.initialize();
			this.statisticsPanel.setAnalysis(analysis);
			this.annotationPanel.setAnalysis(analysis);
			this.typePanel.setAnalysis(analysis);
			this.relationPanel.setAnalysis(analysis);
			this.attributePanel.setAnalysis(analysis);
