...
public class StatisticsPanel extends JPanel implements MouseMotionListener,
	...
	public String getFmeasure(int row) {
		...
		// Get true positive etc counts for a given classification value
		float tp = cm.getTruePositiveCount(row);
		float tn = cm.getTrueNegativeCount(row);
		float fp = cm.getFalsePositiveCount(row);
		float fn = cm.getFalseNegativeCount(row);
		...
		float Bwt = 1;
		/*
		 * B weight of recall vs precision, B>1,or<1, means weight recall, or precision,
		 * more
		 */

		float recall = 0;
		float denominator = tp + fn;
		if (denominator > 0) {
		...
		/* else let precision be 0 */

		float num = (1 + Bwt * Bwt) * recall * precision;
		denominator = (Bwt * Bwt * precision + recall);

		return getResultString(num, denominator);
	}

	// Ensures that the denominator is > 0, and chops floating point
	// display result string to 4 characters.  Returns "*" if
	// denominator is 0.
	...
	public void writeStatisticsXMLToFile() {
		...
		if (resourcedir != null && dname != null
			...
			datestr = StrUtils.replaceNonAlphaNumericCharactersWithDelim(datestr, '_');
