...
public class KTStringSlotMention extends KTSlotMention {
	...
	public KTStringSlotMention(String ehostid, String mentionSlotID,
			String value) {
		this.id = ehostid;
		this.mentionSlotID = mentionSlotID;
		this.stringValue = value;
		
		// 11/13/2017- Trying to acomodate attributes...
		if (this.name == null) {
			this.name = this.mentionSlotID;
		}
		
		if (value.toLowerCase().contains("walk")) {
			int x = 1;
		}
	}
	...
	public void extractInformationXMLFormatSHARP() throws Exception {
		...
		// 2/26/2018
		if ("classification".equals(this.mentionSlotID)) {
			this.stringValue = StrUtils.removeNumberFromMentionClassID(this.stringValue);
		}
	...
	public String toString() {
		return "<KTStringSlotMention:Name=" + this.name + ",StringValue="
				+ this.stringValue + ">";
	}

	public String toXML() {
		String xml = "    <stringSlotMention id=\"" + this.id + "\">\n";
		xml += "        <mentionSlot id=\"" + this.mentionSlotID + "\" />\n";
		xml += "        <stringSlotMentionValue value=\"" + this.stringValue
				+ "\" />\n";
		xml += "    </stringSlotMention>";
		return xml;
	}
