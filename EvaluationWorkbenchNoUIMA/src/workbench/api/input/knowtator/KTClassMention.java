...
public class KTClassMention extends KTSimpleInstance {
	...
	public KTClassMention(String ehostid, String mcid, String mcv, KTAnnotation kta) {
		this.id = ehostid;
		this.name = ehostid;
	...
	public KTClassMention(KnowtatorIO kt, String name, Element node) throws Exception {
		super(kt, name, node);
		kt.classMentions = VUtils.add(kt.classMentions, this);
		extractInformation();
	}

	public KTClassMention(KnowtatorIO kt, String name, Vector v) throws Exception {
		super(kt, name, v);
		kt.classMentions = VUtils.add(kt.classMentions, this);
	...
	public void extractInformationXMLFormatSHARP() throws Exception {
		...
		this.mentionClassID = StrUtils.removeNumberFromMentionClassID(node.getAttributeValue("id"));
		Vector<Element> nodes = JDomUtils.getElementsByName(this.node, "hasSlotMention");
	...
	public void extractInformationLispFormat(Vector v) throws Exception {
		this.mentionClassID = (String) VUtils.assocValueTopLevel("knowtator_mention_class", v);
		this.slotMentionIDs = VUtils.rest(VUtils.assocTopLevel("knowtator_slot_mention", v));
	}
	...
	public void extractInformationXMLFormatOriginal() throws Exception {
		...
		if (OSVNodes != null) {
			for (Element osvnode : OSVNodes) {
				Element rnode = JDomUtils.getElementByName(osvnode, "slot_reference");
				String rvalue = rnode.getText();
				Vector<Element> children = JDomUtils.getElementsByName(osvnode, "value");
	...
	public void resolveReferences() throws Exception {
		...
		if (this.annotationID != null) {
			this.annotation = (KTAnnotation) this.kt.getHashItem(this.annotationID);
		}
	}
		...
		// relations now.
			...
			for (KTSlotMention sm : this.slotMentions) {
				if (sm instanceof KTComplexSlotMention) {
					...
					if (csm.complexSlotClassMention != null && csm.complexSlotClassMention.getAnnotation() != null) {
						KTAnnotation annotation = csm.complexSlotClassMention.getAnnotation();
	...
	public String getValue() throws Exception {
		TypeSystem ts = this.kt.getTypeSystem();
		if (ts != null && this.getMentionClass() != null && this.mentionClass.getType() != null
				&& this.getSlotMentions() != null) {
	...
	public String toString() {
		return "<KTClassMention:" + this.name + "(\"" + this.annotation.getText() + "\">";
	}
	...
	public String toXML() {
		...
		xml += "        <mentionClass id=\"" + this.mentionClassID + "\">" + this.mentionClassValue
				+ "</mentionClass>\n";
		if (this.slotMentionIDs != null) {
