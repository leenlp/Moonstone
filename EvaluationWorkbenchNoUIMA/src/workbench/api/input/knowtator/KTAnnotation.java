...
public class KTAnnotation extends KTSimpleInstance {
	...
	public KTAnnotation(String id, String text, int tstart, int tend, String datestr) {
		this.id = id;
	...
	public KTAnnotation(KnowtatorIO kt, String name, Element node) throws Exception {
		super(kt, name, node);
		extractInformation();
		if (!this.isValid()) {
			return;
		}
		Vector<String> key = KnowtatorIO.getAnnotationHashKey(this.textSource, this.annotatorID,
				kt.getAnalysis().readAnnotationCollectionFileIsPrimary());
		kt.annotations = VUtils.add(kt.annotations, this);
		VUtils.pushHashVector(kt.annotationHash, key, this);
	}

	public KTAnnotation(KnowtatorIO kt, Vector v, String fname) throws Exception {
		super(kt);
		int start = 0, end = 0;
		...
		// longer needed?
		this.ktvector = v;
		this.annotatedMentionID = (String) VUtils.assocValueTopLevel("knowtator_annotated_mention", this.ktvector);
		this.annotatorID = (String) VUtils.assocValueTopLevel("knowtator_annotation_annotator", this.ktvector);
		this.annotatorName = this.annotatorID;
		this.knowtatorAnnotatorName = this.annotatorName;
		this.knowtatorSet = (String) VUtils.assocValueTopLevel("knowtator_set", this.ktvector);
		...
		Vector sv = VUtils.assocTopLevel("knowtator_annotation_span", this.ktvector);
		if (sv != null) {
			sv = VUtils.rest(sv);
		...
		this.text = (String) VUtils.assocValueTopLevel("knowtator_annotation_text", this.ktvector);
		if (start > 0 && end - start <= 1) {
			this.isLengthIndicatedPossibleRelation = true;
		}
		this.textSource = (String) VUtils.assocValueTopLevel("knowtator_annotation_text_source", this.ktvector);
	...
	public void extractInformationXMLFormatSHARP() throws Exception {
		...
		if (this.annotatorName != null && this.kt.getAnalysis().readAnnotationCollectionFileIsPrimary()
		...
		/****
		 * // 3/10/2016 TEST 3/29/2017: In handling adjudication files, the name is
		 * important. I need to come up with a permanent solution for this. String psstr
		 * = (this.kt.getAnalysis() .readAnnotationCollectionFileIsPrimary() ? "primary"
		 * : "secondary"); if (this.annotatorID != null) { this.annotatorID =
		 * this.annotatorID + "-" + psstr; this.annotatorID =
		 * this.annotatorID.toLowerCase(); }
		 * 
		...
		/****
		 * 3/29/2017: Removed temporarily. Annotator name as recorded in the annotation
		 * files is important for adjudication tasks in particular. if
		 * (this.kt.annotator != null) { this.annotatorID = this.kt.annotator.getName();
		 * this.annotatorName = this.kt.annotator.getName(); }
		 *****/
	}

	public void extractInformationXMLFormatOriginal() throws Exception {
		Vector<Element> OSVNodes = JDomUtils.getElementsByName(this.node, "own_slot_value");
		if (OSVNodes != null) {
			for (Element osvnode : OSVNodes) {
				Element cnode = JDomUtils.getElementByName(osvnode, "slot_reference");
	...
	public void resolveReferences() throws Exception {
		if (this.annotatedMentionID != null) {
			...
			this.annotatedMention = (KTClassMention) this.kt.getHashItem(annotatedMentionID);
			this.annotatedMention.annotation = this;
		}
	}

	public String toString() {
		return "<KTAnnotation:" + this.name + ",Text=\"" + this.text + "\",Spans=" + this.getSpanStrings();
	}
	...
	public Annotation extractSnippet(AnnotationCollection ac, Hashtable<String, Annotation> spanhash) throws Exception {
		int x = 1;
		try {
			Type type = null;
			...
			if (ac.getSourceTextName().contains("71m3c")) {
				x = 1;
			}

			if (this.getSnippet() == null && this.getText() != null && this.getTextStarts() != null && mention != null
			...
			// && mention.getMentionClass() != null
				...
				if (this.getAnnotatedMention().getMentionClass() != null) {
					type = ts.getType(this.getAnnotatedMention().getMentionClass().getName());
				...
				if (type != null) {
					...
					if (mention != null && mention.getSlotMentions() != null) {
						for (KTSlotMention sm : mention.getSlotMentions()) {
							...
							if (value.toString().toLowerCase().contains("language")) {
								x = 1;
							}
							...
							if (value != null) {
								value = ac.getAnalysis().getEHostConceptConversion(value.toString());
								...
								if (attr == null) {
									...
									String rastr = ts.getRootType().getFullname() + ":" + aname;
			...
			if (snippet != null && type != null && snippet.getClassificationValue() == null
					&& mention.getMentionClassID() != null) {
				String cname = "classification";
				String dcn = this.getKt().getAnalysis().getKnownledgeEngine().getStartupParameters()
						.getPropertyValue("DefaultClassificationPropertyName");
				if (dcn != null) {
					cname = dcn;
					if (cname.toLowerCase().contains("language")) {
						x = 1;
					}
				}
				String value = mention.getMentionClassID();
				value = snippet.getAnalysis().getEHostConceptConversion(value);

				// 12/15/2017 test: temporarily removing...
				ts.getRootType().addAttribute(cname);
				Classification c = new Classification(type, cname);
				snippet.putAttributeValue(c, value);
				snippet.setClassificationValue(value);
				c.addValue(value);
			}
	...
	public static boolean isNonOverlapping(KTAnnotation a1, KTAnnotation a2) {
		if (a1.getTextEnd() < a2.getTextStart() || a2.getTextEnd() < a1.getTextStart()) {
			return true;
		}
		return false;
	}
	...
	public KTSlotMention getSlotMention(String sname) {
		...
		if (cm.getSlotMentions() != null) {
			for (KTSlotMention sm : cm.getSlotMentions()) {
				...
				if (sm instanceof KTComplexSlotMention) {
					KTComplexSlotMention csm = (KTComplexSlotMention) sm;
					csm.complexSlotClassMention.annotation.getSlotMention(sname);
				}
			}
		}
		this.setVisited(false);
			...
			for (KTSlotMention sm : cm.getSlotMentions()) {
				if (sname.equals(sm.mentionSlotID)) {
					if (sm instanceof KTComplexSlotMention) {
						KTComplexSlotMention csm = (KTComplexSlotMention) sm;
						KTAnnotation annotation = csm.complexSlotClassMention.annotation;
						return annotation.getSlotValuedAnnotation(slotNames, sindex + 1);
					}
				}
			}
		}
	...
	public String toXML() {
		...
		xml += "        <annotator id=\"" + this.annotatorID + "\">" + this.annotatorID + "</annotator>\n";
		xml += "        <span start=\"" + this.textStarts[0] + "\" end=\"" + this.textEnds[0] + "\" />\n";
		String htmltext = "NOTEXT";
		if (this.getText() != null) {
			htmltext = StrUtils.replaceNonAlphaNumericCharactersWithSpaces(this.getText());
		}
