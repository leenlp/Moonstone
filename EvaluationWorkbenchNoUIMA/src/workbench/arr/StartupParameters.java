...
public class StartupParameters {
	...
	String resultsOutputDirectory = null;
	...
	public static Vector<String> AnnotatorTypes = VUtils.arrayToVector(
			new String[] { "GrAF", "Knowtator", "XMI", "CSV", "PIPELINE", "I2B2", "PyConText", "CLEF", "CLEF2" });
	...
	public static String TextInputDirectoryName = "TextInputDirectory";
	public static String ResultsOutputDirectoryName = "ResultsOutputDirectory";
	...
	public StartupParameters(EvaluationWorkbench arrTool, boolean doread, boolean withbail) throws Exception {
		this.arrTool = arrTool;
		arrTool.startupParameters = this;
		if (doread) {
			readParameters(withbail);
		}
	}
	...
	public String getErrors() {
		...
		if (!StrUtils.containsIgnoreCase(AnnotatorTypes, this.inputTypeFirstAnnotator)) {
			return "WorkbenchAnnotationFileTypeFirstAnnotator must be defined, and one of: " + AnnotatorTypes + ".\n";
		}
		if (!StrUtils.containsIgnoreCase(AnnotatorTypes, this.inputTypeSecondAnnotator)) {
			return "WorkbenchAnnotationFileTypeSecondAnnotator must be defined, and one of: " + AnnotatorTypes + ".\n";
		}
		if (this.textInputDirectory != null) {
			file = new File(this.textInputDirectory);
		}
		if (file == null || !file.exists() || !file.isDirectory()) {
			if (!canIgnoreParameter(this.inputTypeFirstAnnotator, "TextInputDirectory")) {
		...
		if (this.isInputTypeFirstAnnotatorKnowtator()) {
			if (this.knowtatorPinsFileFirstAnnotator != null) {
				...
				if (!file.exists()) {
					return "KnowtatorPinsFileFirstAnnotator file: " + this.knowtatorPinsFileFirstAnnotator
							+ " does not exist\n";
				}
			}
		}
		if (this.isInputTypeSecondAnnotatorKnowtator()) {
			if (this.knowtatorPinsFileSecondAnnotator != null) {
				...
				if (!file.exists()) {
					return "KnowtatorPinsFileSecondAnnotator file: " + this.knowtatorPinsFileSecondAnnotator
							+ " does not exist\n";
				}
			}
		}
		if ((this.isInputTypeFirstAnnotatorKnowtator() || this.isInputTypeSecondAnnotatorKnowtator())
	...
	String readParameters(boolean withbail) throws Exception {
		String str = null;
		tsl.knowledge.engine.StartupParameters sp = KnowledgeEngine.getCurrentKnowledgeEngine().getStartupParameters();
		...
		this.inputTypeFirstAnnotator = sp.getPropertyValue(WorkbenchAnnotationFileTypeFirstAnnotator);
		this.inputTypeSecondAnnotator = sp.getPropertyValue(WorkbenchAnnotationFileTypeSecondAnnotator);
		if (!StrUtils.containsIgnoreCase(AnnotatorTypes, inputTypeFirstAnnotator)
				|| !StrUtils.containsIgnoreCase(AnnotatorTypes, inputTypeSecondAnnotator)) {
			return this.returnWithError("Missing first/second annotator");
		}
		boolean usesTextDirectory = !(this.isInputTypeFirstAnnotatorCSV() || this.isInputTypeFirstAnnotatorPyConText());
		str = sp.getPropertyValue(TextInputDirectoryName);
		if (str != null) {
			if (FUtils.isRelativePath(str)) {
				str = sp.getRootDirectory() + File.separatorChar + str;
			}
		...
		str = sp.getPropertyValue(ResultsOutputDirectoryName);
		if (str != null) {
			if (FUtils.isRelativePath(str)) {
				str = sp.getRootDirectory() + File.separatorChar + str;
			}
			File file = new File(str);
			if (file.exists() && file.isDirectory()) {
				this.resultsOutputDirectory = file.getAbsolutePath();
			}
		} else {
			if (usesTextDirectory) {
				if (withbail) {
					return this.returnWithError("Missing results directory");
				}
			}
		}
		...
		if (str != null) {
			...
			File file = new File(str);
			if (file.exists() && file.isDirectory()) {
				this.annotationInputDirectoryFirstAnnotator = file.getAbsolutePath();
				if (this.textInputDirectory == null) {
		...
		if (str != null) {
			...
			File file = new File(str);
			if (file.exists() && file.isDirectory()) {
				this.annotationInputDirectorySecondAnnotator = file.getAbsolutePath();
				if (this.textInputDirectory == null) {
		...
		if (this.isInputTypeFirstAnnotatorKnowtator()) {
			if (this.knowtatorPinsFileFirstAnnotator == null && this.annotationInputDirectoryFirstAnnotator == null) {
				if (withbail) {
		...
		if (this.isInputTypeSecondAnnotatorKnowtator()) {
			if (this.knowtatorPinsFileSecondAnnotator == null && this.annotationInputDirectorySecondAnnotator == null) {
				if (withbail) {
					return this.returnWithError("Missing input directory");
				}
			}
		...
		if (this.isInputTypeFirstAnnotatorKnowtator() || this.isInputTypeSecondAnnotatorKnowtator()) {
			String knowtatorSchemaFile = sp.getPropertyValue(WorkbenchKnowtatorSchemaFile);

			if (FUtils.isRelativePath(knowtatorSchemaFile)) {
				knowtatorSchemaFile = sp.getRootDirectory() + File.separatorChar + knowtatorSchemaFile;
			}
		...
		if (str != null) {
			this.permitEquivalentClassificationNames = Boolean.parseBoolean(str);
		}
		str = sp.getPropertyValue(StateAttributes);
		if (str != null) {
			arrTool.stateAttributeNames = VUtils.vectorToStringArray(StrUtils.stringList(str, ','));
		}
	...
	public void storeParameterProperties() throws Exception {
		Properties p = KnowledgeEngine.getCurrentKnowledgeEngine().getStartupParameters().getProperties();
		if (this.firstAnnotatorName != null) {
		...
		if (this.inputTypeFirstAnnotator != null) {
			p.setProperty(WorkbenchAnnotationFileTypeFirstAnnotator, this.inputTypeFirstAnnotator);
		}
		if (this.inputTypeSecondAnnotator != null) {
			p.setProperty(WorkbenchAnnotationFileTypeSecondAnnotator, this.inputTypeSecondAnnotator);
		}
		if (this.textInputDirectory != null) {
			p.setProperty(TextInputDirectoryName, this.textInputDirectory);
		}
		if (this.annotationInputDirectoryFirstAnnotator != null) {
			p.setProperty(AnnotationInputDirectoryFirstAnnotator, this.annotationInputDirectoryFirstAnnotator);
		}
		if (this.annotationInputDirectorySecondAnnotator != null) {
			p.setProperty(AnnotationInputDirectorySecondAnnotator, this.annotationInputDirectorySecondAnnotator);
		}
		if (this.knowtatorPinsFileFirstAnnotator != null) {
			p.setProperty(KnowtatorPinsFileFirstAnnotator, this.knowtatorPinsFileFirstAnnotator);
		}
		if (this.knowtatorPinsFileSecondAnnotator != null) {
			p.setProperty(KnowtatorPinsFileSecondAnnotator, this.knowtatorPinsFileSecondAnnotator);
		}
		if (this.clefAnnotationOutputDirectory != null) {
			p.setProperty(CLEFAnnotationOutputDirectory, this.clefAnnotationOutputDirectory);
		}
		if (this.arrTool.typeSystem != null) {
			TypeSystem ts = this.arrTool.typeSystem;
			if (ts.getDefaultClassificationProperties() != null) {
				String props = StrUtils.stringListConcat(ts.getDefaultClassificationProperties(), ",");
				p.setProperty(DefaultClassificationProperties, props);
			}
		}
		if (this.knowtatorSchemaFile != null) {
			p.setProperty(WorkbenchKnowtatorSchemaFile, this.knowtatorSchemaFile);
		}
		if (this.arrTool.annotatorNameValidators != null) {
			String str = StrUtils.stringListConcat(this.arrTool.annotatorNameValidators, ",");
		...
		p.setProperty(StoreGeneralStatisticsModels, Boolean.toString(this.isStoreGeneralStatisticsModels()));
	}
	...
	public void writeParametersToStartupFile() throws Exception {
		...
		String fname = this.workbenchDirectory + File.separatorChar + StartupParameterFileName;
	...
	public Annotator getFirstAnnotator() {
		if (this.firstAnnotator == null && this.firstAnnotatorName != null) {
			this.firstAnnotator = new Annotator(this.arrTool.analysis, this.firstAnnotatorName);
		}
	...
	public Annotator getSecondAnnotator() {
		if (this.secondAnnotator == null && this.secondAnnotatorName != null) {
			this.secondAnnotator = new Annotator(this.arrTool.analysis, this.secondAnnotatorName);
		}
	...
	public void setPermitAllDefaultClassicationNames(boolean permitAllDefaultClassicationNames) {
		this.permitAllDefaultClassicationNames = permitAllDefaultClassicationNames;
	}

	public void setAnnotationInputDirectoryFirstAnnotator(String annotationInputDirectoryFirstAnnotator) {
		this.annotationInputDirectoryFirstAnnotator = annotationInputDirectoryFirstAnnotator;
	}

	public void setAnnotationInputDirectorySecondAnnotator(String annotationInputDirectorySecondAnnotator) {
		this.annotationInputDirectorySecondAnnotator = annotationInputDirectorySecondAnnotator;
	}

	public Properties getProperties() {
		return KnowledgeEngine.getCurrentKnowledgeEngine().getStartupParameters().getProperties();
	}
