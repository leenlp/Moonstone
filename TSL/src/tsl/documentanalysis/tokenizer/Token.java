...
public class Token {
	...
	public static int HTMLQUOTEDELIM = 16;

	// Word subtypes
	public static int FIRSTUPPERCASE = 100;

	public static int ALLUPPERCASE = 101;

	public static int ALLLOWERCASE = 102;

	public static int REGEX = 103;

	// Delimiter / punctuation subtypes
	public static int COLON = 200;

	public static int SEMICOLON = 201;

	public static int PERIOD = 202;

	public static int QUESTION = 203;

	public static int EXCLAMATION = 204;

	// Other punctuation subtypes
	public static int COMMA = 206;

	public static int SLASH = 207;

	public static int SINGLEQUOTE = 208;

	public static int DOUBLEQUOTE = 209;

	public static int UNKNOWN = 210;

	// Added 10/2/2008 so I can parse surface pattern element strings, e.g.
	// "np<restcond>[amalgam]"
	public static int LEFTARROW = 211;

	public static int RIGHTARROW = 212;

	public static int LEFTBRACKET = 213;

	public static int RIGHTBRACKET = 214;

	public static int LEFTPAREN = 215;

	public static int RIGHTPAREN = 216;

	public static int SPACE = 217;

	public static int NEWLINE = 218;

	public static int TAB = 219;

	public static int CHECKED = 220;

	public static int UNCHECKED = 221;

	public static int POSSESSIVE_CHARACTER = 300;

	public static int[] tokenTypes = { WORD, WHITESPACE, PUNCTUATION, QUOTEDSTRING, DEIDENTIFIEDBLOCK, NUMBER, TITLE,
			ABBREVIATION, ENDDOCUMENTDELIM, DATE, TIME, URL, LONGNEWLINE, ASTERISK, CHECKBOX, UNDERLINE };

	public static String[] tokenNames = { "WORD", "WHITESPACE", "PUNCTUATION", "QUOTE", "DEIDENT", "NUMBER", "TITLE",
			"ABBREVIATION", "ENDDOCDELIM", "DATE", "TIME", "URL", "LONGNEWLINE", "ASTERISK", "CHECKBOX", "UNDERLINE" };

	public static String[] longNewlines = { "\n\n", "\n\f\n\f", "\f\n\f\n", "\r\n\r\n" };

	public static int[] longNewlineTypes = { LONGNEWLINE, LONGNEWLINE, LONGNEWLINE, LONGNEWLINE };

	public static String[] longNewlineNames = { "double newline", "double newline", "double newline" };
	...
	public static int[] punctuationTypes = { COLON, SEMICOLON, COMMA, PERIOD, QUESTION, EXCLAMATION, SLASH, SINGLEQUOTE,
			DOUBLEQUOTE, LEFTARROW, RIGHTARROW, LEFTBRACKET, RIGHTBRACKET, LEFTPAREN, RIGHTPAREN, SPACE, NEWLINE, TAB,
			ASTERISK };

	public static String[] punctuationNames = { "colon", "semicolon", "comma", "period", "question", "exclamation",
			"slash", "singlequote", "doublequote", "leftarrow", "rightarrow", "leftbracket", "rightbracket",
			"leftparen", "rightparen", "space", "newline", "tab", "asterisk" };

	public static String[] NonDelimitingPunctuationNames = { "hyphen", "slash", "leftparen", "rightparen", "equals",
			"space", "tab", "at" };
	...
	public static String[] abbreviations = { "Ph.D.", "D.M.D.", "D.D.S.", "M.D.", "q.", "b.i.d.", "m.g.", "g.", "a.m.",
			"p.m.", "e.g.", "p.o.", "t.i.d.", "p.r.n.", "q.h.s", "q.i.d", "q.p.m", "q.6h.", "q.7h.", "q.8h.", "dtrs.", "inpt."};

	static String endDocumentDelim = "<ENDDOC>";
	static String HTMLQuoteDelim = "&quot;";
	...
	public Token(Token token) {
		this(token.getType(), token.getString(), token.getStart(), token.getEnd(), token.getValue());
	...
	public Token(int type, String str, int start, int end, Object value) {
		try {
			this.type = type;
			if (this.type < tokenNames.length) {
				...
				if (Character.isLetter(this.string.charAt(0))) {
					...
					this.subtype = determineSubtype(this.string, Whitespace, WhitespaceTypes);
				} else if (type == PUNCTUATION) {
					this.subtype = determineSubtype(this.string, punctuation, punctuationTypes);
				} else if (type == NUMBER) {
					try {
	...
	public static void readTokensFromInput(Document report) {
		...
		if (report.getText() != null) {
			...
			while ((t = readNextTokenFromInput(report)) != null) {
				if (t != lastToken) {
					t.index = index++;
					// 11/17/2017: Allowing commas to have a valid word index
	...
	public static Token readNextTokenFromInput(Document report) {
		...
		} else if ((str = readString(report, endDocumentDelim)) != null) {
			end = report.textIndex - 1;
			token = new Token(ENDDOCUMENTDELIM, str, start, end, null);
			// 4/11/2018 TEST
		} else if ((str = readString(report, HTMLQuoteDelim)) != null) {
			end = report.textIndex - 1;
	...
	public boolean isWordSurrogate() {
		return (isWord() || isNumber() || isDeidentifiedBlock() || isAbbreviation() || isTitle() || isQuotedString()
				|| isURL() || isCheckbox()

				// 11/18/2017: Allowing commas to be treated as words (WATCH FOR SIDE EFFECTS!!)
				|| isComma());
	}
	...
	public boolean isNonDelimitingCharacter() {
		return this.string.length() == 1 && isNonDelimitingCharacter(this.string.charAt(0));
	}
	...
	public boolean isHTMLQuoteDelim() {
		return this.type == HTMLQUOTEDELIM;
	}
	...
	public static Token peekNextToken(Document report, String str) {
		if (report.tokenIndex < report.getTokens().size()) {
			Token token = (Token) report.getTokens().elementAt(report.tokenIndex);
	...
	public static Token peekFollowingToken(Document report) {
		if (report.getTokenIndex() < report.getTokens().size() - 1) {
			return (Token) report.getTokens().elementAt(report.getTokenIndex() + 1);
		}
		return null;
	}
	...
	static String readWord(Document report) {
		...
		if (Character.isLetter(ch) || Character.isDigit(ch)) {
			...
			while (inword) {
				...
				if (Character.isLetter(ch) || Character.isDigit(ch) || isWordCharacterStandin(ch)
						|| (!Character.isWhitespace(ch) && !isSignificantPunctuation(report, ch)
								&& (Character.isLetter(fch) || Character.isDigit(fch)))) {
					sb.append(ch);
					report.textIndex++;
				} else {
					inword = false;
	...
	static String readNumber(Document report) {
		...
		if (Character.isDigit(ch)) {
			...
			while (inword) {
				...
				if (Character.isDigit(ch) || (!founddelimiter && (ch == '.' || ch == '/') && Character.isDigit(fch))) {
					if (ch == '.' || ch == '/') {
						founddelimiter = true;
					}
					sb.append(ch);
	...
	static String readPunctuation(Document report) {
		...
		if (ch != (char) -1 && !(Character.isDigit(ch) || Character.isLetter(ch) || Character.isWhitespace(ch))) {
	...
	static String readUnderline(Document report) {
		...
		if (underlineCount > 1) {
			underline = report.getText().substring(oldindex, report.getTextIndex());
		} else {
	...
	static String readWhitespace(Document report) {
		...
		char ch = peekNextChar(report);
		if (ch != (char) -1 && !(Character.isDigit(ch) || Character.isLetter(ch))) {
	...
	static String readDelimitedString(Document report, String start, String end, int maxlen) {
		...
		if (report != null && report.getText() != null) {
			...
			for (int i = 0; i < titles.length; i++) {
				...
				if (remaining > title.length()
						&& title.regionMatches(0, report.getText(), report.textIndex, title.length())) {
		...
		if (report != null && report.getText() != null) {
			...
			for (int i = 0; i < abbreviations.length; i++) {
				...
				if (remaining > abbreviation.length()
						&& abbreviation.regionMatches(0, report.getText(), report.textIndex, abbreviation.length())) {
	...
	static String readString(Document report, String str) {
		char firstchar = str.charAt(0);
		int strlen = str.length();
		if (report.lengthToTextEnd() >= strlen && report.getText().charAt(report.textIndex) == firstchar) {
			int remaining = report.getText().length() - report.textIndex;
			if (remaining >= strlen && str.regionMatches(0, report.getText(), report.textIndex, strlen)) {
				report.textIndex += strlen;
				return str;
			}
		}
		return null;
	...
	public static boolean peekNextString(Document report, String str) {
		if (peekNextChar(report) == str.charAt(0) && report.textIndex + str.length() <= report.getText().length()) {
			String substr = report.getText().substring(report.textIndex, report.textIndex + str.length());
	...
	public static Vector<Token> gatherWordTokens(Vector tokens) {
		...
		if (tokens != null) {
			for (Enumeration e = tokens.elements(); e.hasMoreElements();) {
				...
				// 6/14/2018:  This is unexpected failures, with documents with lots of parenthetic comments...
				insideParens = false;
				
				if (!insideParens && (token.isWordSurrogate() || token.isComma())) {
	...
	public static String determinePhraseType(Vector<Token> tokens) {
		...
		if (tokens != null) {
			...
			if (lastWord == null || lastWord.isNoun()) {
				if (firstWord == null || firstWord.isNoun() || firstWord.isAdjective()) {
					ptype = "np";
	...
	public static Vector gatherWordSubstrings(String str, int len) {
		...
		if (words != null) {
			for (int i = 0; i < words.size(); i++) {
				...
				for (int j = end; j >= i; j--) {
					String substr = StrUtils.stringListConcat(VUtils.subVector(words, i, j + 1), " ");
					if (substr != null) {
			...
			if (tokens != null) {
				for (Enumeration e = tokens.elements(); e.hasMoreElements();) {
					...
					if (token.isWordSurrogate()) {
						...
						if (word != null) {
							if (word.getPartsOfSpeech().contains("noun") || word.getPartsOfSpeech().contains("adj")) {
	...
	public Object clone() {
		Token newToken = new Token(this.type, this.string, this.start, this.end, this.value);
		return newToken;
	}
	...
	public boolean isSentenceDelimiter() {
		...
		boolean result = (this.getType() == LONGNEWLINE || (this.isPunctuation()
				&& (this.subtype == PERIOD || this.subtype == SEMICOLON || this.subtype == COLON
	...
	public boolean isAVPairDelimiter() {
		boolean result = (this.isPunctuation() && (this.getSubtype() == COLON || this.getSubtype() == QUESTION));
		return result;
	}
	...
	public void determineWordSubtype() {
		...
		this.subtype = (alluppercase ? ALLUPPERCASE : (firstuppercase ? FIRSTUPPERCASE : ALLLOWERCASE));
	}
