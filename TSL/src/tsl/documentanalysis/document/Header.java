...
public class Header extends DocumentItemConstant {
	...
	private static Vector<String> permissibleHeaderPunctuation = VUtils.arrayToVector(new String[] { "/", "(", ")" });
	...
	public String toString() {
		return "<\"" + this.text + ",TokenStart=" + this.textStartTokenIndex + ",TokenEnd=" + this.textEndTokenIndex
				+ ",Content=\"***\"])";
	}

	private static int MaxHeaderTokenLength = 16;

	static Vector<Token> getHeaderTokens(Document document, boolean useSectionHeaderHeuristics,
			boolean inhibitFirstWordUppercaseHeaders) {
		...
		if (startToken != null) {
			hasSufficientPrecedingWhitespace = hasSufficientPrecedingWhitespace(docText, startToken.getStart());
			atNewline = startToken.getStart() == 0 || docText.charAt(startToken.getStart() - 1) == '\n';
		}

		Token endToken = null;

		if (!hasSufficientPrecedingWhitespace || startToken == null || document.atEnd()) {
		...
		if (startToken != null && document.getHeaderHash() != null) {
			Vector<String> possibleHeaders = document.getHeaderHash().get(startToken.getString());
			if (possibleHeaders != null) {
				for (String possibleHeader : possibleHeaders) {
					if (StrUtils.stringEquals(docText, possibleHeader, startToken.getStart())) {
						matchedHeaderEnd = startToken.getStart() + possibleHeader.length() - 1;
		...
		} else if (useSectionHeaderHeuristics && hasSufficientPrecedingWhitespace
		...
		// && startToken.isFirstuppercase()
			...
			boolean wordsAllCompleteUppercase = true;
			...
			for (int i = startIndex; !atEnd && i < document.getTokens().size()
				...
				if (token.isWord() || (i > startToken.getIndex() && token.isNumber())
						|| (token.isPunctuation() && isPermissibleHeaderPunctuation(token.getString()))) {
					endTokenIndex = i;
					if (!((token.isFirstuppercase()) || permissibleInLineWords.contains(token.getString()))) {
						wordsAllFirstUppercase = false;
					}

					// 6/27/2018
					if (!(token.isAllUpperCase() || permissibleInLineWords.contains(token.getString()))) {
						wordsAllCompleteUppercase = false;
					}
				} else {
					atEnd = true;
				}
			...
			if (endTokenIndex >= startToken.getIndex() && distance <= MaxHeaderTokenLength) {
				hasSimpleBeginning = startToken.isFirstuppercase();
				// 6/27/2018
				hasHeaderEnding = (endsWithHeaderDelimiter
						|| (atNewline && endChar == '\n' && (wordsAllCompleteUppercase
								|| (wordsAllFirstUppercase && !inhibitFirstWordUppercaseHeaders))));
				if ((hasSimpleBeginning || hasPatternBeginning) && hasHeaderEnding) {
		...
		if (endToken != null) {
			try {
				headerTokens = new Vector(document.getTokens().subList(startToken.getIndex(), endToken.getIndex() + 2));
	...
	static Vector<Token> getHeaderTokens_BEFORE_6_27_2018(Document document, boolean useSectionHeaderHeuristics,
			boolean inhibitFirstUpper) {
		Vector<Token> headerTokens = null;
		char endChar = (char) -1;
		boolean endsWithHeaderDelimiter = false;
		boolean hasPatternBeginning = false;
		...
		// && startToken.isFirstuppercase()
			...
			if (endTokenIndex >= startToken.getIndex() && distance <= MaxHeaderTokenLength) {
				hasSimpleBeginning = startToken.isFirstuppercase();
				hasHeaderEnding = (endsWithHeaderDelimiter || (atNewline && endChar == '\n' && wordsAllFirstUppercase));
				if ((hasSimpleBeginning || hasPatternBeginning) && hasHeaderEnding) {
					endToken = document.getTokens().elementAt(endTokenIndex);
				}
			}
	...
	private static boolean hasSufficientPrecedingWhitespace(String text, int cindex) {
		int spaceCount = 0;
		for (int i = cindex - 1; i >= 0; i--) {
			char c = text.charAt(i);
			
			// 6/28/2018:  For Vanderbilt headers starting with a variable number of '-'s
			if (c == '-') {
				continue;
			}
	...
	public static Header readNextHeader(Document document, boolean useSectionHeaderHeuristics,
			boolean inhibitFirstWordUppercaseHeaders) {
		int x = 1;
		Vector tokens = getHeaderTokens(document, useSectionHeaderHeuristics, inhibitFirstWordUppercaseHeaders);
		Header header = null;
		if (tokens != null) {
	...
	public Vector<Token> getSpaceDelimitedStringTokens() {
		...
		if (firstWordTokenIndex >= 0) {
			tokens = (Vector<Token>) VUtils.subVector(doc.getTokens(), firstWordTokenIndex, lastWordTokenIndex + 1);
	...
	public Vector<Token> getTextTokens() {
		Vector<Token> tokens = VUtils.subVector(this.document.getTokens(), this.getTextStartTokenIndex(),
				this.getTextEndTokenIndex() + 1);
		return tokens;
	}
