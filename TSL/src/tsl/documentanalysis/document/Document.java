...
public class Document extends DocumentItemConstant {
	...
	public static int[][] DateRanges = new int[][] { { -30, 0 }, { 1, 7 }, { 8, 30 }, { 31, 180 }, { 181, 365 },
			{ 366, 1000000 } };
	...
	public void gatherHeaders(boolean useSectionHeaderHeuristics) {
		...
		if (this.tokens != null && !this.tokens.isEmpty()) {
			...
			int hindex = 0;
			Header header = null;
			boolean inhibitFirstWordUppercaseHeaders = KnowledgeEngine.currentKnowledgeEngine.getStartupParameters()
					.isPropertyTrue("InhibitFirstWordUppercaseHeaders");
			while (!atEnd) {
				header = Header.readNextHeader(this, useSectionHeaderHeuristics, inhibitFirstWordUppercaseHeaders);
				if (header != null) {
	...
	public void analyzeContent(boolean useSectionHeaderHeuristics) {
		...
		if (this.headers != null) {
			for (Header header : this.headers) {
				header.textStartTokenIndex = header.getTokens().lastElement().getIndex() + 1;
				int hindex = header.getReportIndex();
				if (hindex < this.getHeaders().size() - 1) {
					Header next = this.getHeaders().elementAt(hindex + 1);
					header.textEndTokenIndex = next.getTokens().firstElement().getIndex() - 1;
				} else {
		...
		if (this.headers != null) {
			for (Header h : this.headers) {
				if (!h.equals(s.getHeader())) {
					...
					boolean overlaps = SeqUtils.overlaps(hstart, hend, sstart, send);
					if (overlaps) {
						return true;
					}
				}
	...
	public Vector<Sentence> getSentencesContainingWord(String word) {
		...
		if (index > 0) {
			...
			Vector<Sentence> v = (Vector<Sentence>) wordSentenceHash.get(words.firstElement());
			if (v != null) {
	...
	public Vector getSentenceStrings(boolean useSectionHeaderHeuristics) {
		...
		Vector strings = null;
		if (this.allSentences != null) {
			for (Enumeration e = this.allSentences.elements(); e.hasMoreElements();) {
	...
	public boolean atEnd() {
		if (this.getText() != null && this.getTextIndex() >= this.getText().length()) {
			return true;
		}
	...
	public String getText() {
		if (this.text == null && this.documentID >= 0) {
			this.text = new MySQLAPI("").readDocumentText(this.documentID);
		}
	...
	public boolean equals(Object o) {
		if (o instanceof Document) {
			...
			if (this.getAbsoluteFilePath() != null && this.getAbsoluteFilePath().equals(d.getAbsoluteFilePath())) {
				return true;
			}
		}
		return false;
	...
	public int convertTextIndexToTokenIndex(int textIndex) {
		if (this.tokens != null) {
			for (int i = 0; i < this.tokens.size(); i++) {
				...
				if (token.getStart() >= textIndex && token.getEnd() <= textIndex) {
					return i;
				}
			}
		}
	...
	public int getTokens(int startindex, int starttext, int endtext, Vector<Token> tokens) {
	...
	public boolean extractPatientNameAndDatesFromFirstLine(String[] dateStringFormats) {
		...
		if (patindex != -1 && patindex < admitindex && admitindex < typeindex && typeindex < dictateindex) {
			...
			this.admitDateString = line.substring(admitindex, admitindex + endindex).trim();
			typeindex += "DocumentType:".length();
			this.documentType = line.substring(typeindex, dictateindex - 1).trim();
			dictateindex += "Date:".length();
			this.dictateDateString = StrUtils.getStringToWhitespace(line, dictateindex);

			// Before 12/21/2016
			// Date admitdate = TimeUtils.getDateFromString(this.admitDateString,
			// admitDateFormat);

			Date admitdate = TimeUtils.getDateFromString(this.admitDateString, dateStringFormats);
			...
			Date dictatedate = TimeUtils.getDateFromString(this.dictateDateString, dateStringFormats);
	...
	public boolean extractPatientNameAndDatesFromFirstLine_BEFORE_12_12_2016() {
		...
		if (patindex != -1 && patindex < admitindex && admitindex < typeindex && typeindex < dictateindex) {
			...
			this.patientName = line.substring(patindex, admitindex - 1).trim();
			admitindex += "AdmitDay:".length();
			this.admitDateString = line.substring(admitindex, typeindex - 1).trim();
			typeindex += "DocumentType:".length();
			this.documentType = line.substring(typeindex, dictateindex - 1).trim();
			dictateindex += "Date:".length();
			this.dictateDateString = StrUtils.getStringToWhitespace(line, dictateindex);
	...
	public int getAbsoluteAdmitDay() {
		int aday = 0;
		if (this.admitDateDay > 0 && this.admitDateYear > 0 && this.admitDateMonth > 0) {
			aday = this.admitDateYear * 356 + this.admitDateMonth * 30 + this.admitDateDay;
		}
		return aday;
	}

	public int getAbsoluteDictationDay() {
		int dday = this.dictateDateYear * 356 + this.dictateDateMonth * 30 + this.dictateDateDay;
		return dday;
	}
	...
	public static String extractPatientNameFromReportName(String fname, String delim) {
		if (fname != null) {
			int index1 = fname.indexOf(delim);
			if (index1 > 0) {
	...
	public int getAdmitDateRangeIndex() {
		if (this.admitDateRangeIndex == -1 && this.admitDateDay > 0) {
			this.admitDictateDayDifference = this.getAdmitDictationDayDifference();
			if (this.admitDictateDayDifference >= 0) {
				for (int i = 0; i < DateRanges.length; i++) {
					int[] dr = DateRanges[i];
					if (dr[0] <= this.admitDictateDayDifference && this.admitDictateDayDifference <= dr[1]) {
