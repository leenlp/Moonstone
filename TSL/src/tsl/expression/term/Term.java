...
public class Term extends Expression {
	...
	protected Vector<PropertySentence> propertySentences = null;
	...
	// Inherit relations and default properties from ontology.
	public void gatherHeritableRelations(Vector<Term> terms) {
		Vector<RelationSentence> rsents = null;
		TypeConstant tc = this.getType();
		if (tc != null && tc.getUnifiableTypeRelationSentences() != null) {
			for (TypeRelationSentence trs : tc.getUnifiableTypeRelationSentences()) {
				for (Term t : terms) {
					if (t.getType().isSubsumedBy(trs.getModifierType())) {
						RelationSentence rs = RelationSentence.createRelationSentence(trs.getRelation(), this, t);
	...
	public void gatherHeritableProperties() {
		TypeConstant tc = this.getType();
		if (tc != null && tc.getUnifiablePropertySentences() != null) {
			for (PropertySentence ps : tc.getUnifiablePropertySentences()) {
				Object value = Term.unpack(ps.getModifier());
				this.setProperty(ps.getSubject().getName(), value);
			}
		}
	}
